...
if ( ! function_exists( 'educbt_class_level_name' ) ) {
    /**
     * Strip the arm suffix from a class display_name so it shows as a level
     * (e.g. "JS1 A" -> "JS1", "SS2 Science B" -> "SS2 Science").
     *
     * @param string $display_name The class display_name from the database.
     * @return string The level name without the arm.
     */
    function educbt_class_level_name( string $display_name ): string {
        $name = trim( $display_name );
        if ( $name === '' ) {
            return '';
        }
        // Strip a trailing single uppercase letter (the arm): "JS1 A" -> "JS1"
        // Also handle "JS1A" without space.
        $name = preg_replace( '/\s+[A-Z]$/', '', $name );
        $name = preg_replace( '/([0-9])([A-Z])$/', '$1', $name );
        return trim( $name );
    }
}
