...
if ( ! empty( $my_subject_ids ) ) {
    ...
    $live = (array) $wpdb->get_results(
        $wpdb->prepare(
            "SELECT p.id, p.scheduled_at, p.subject_id, p.requires_access_code, p.access_code,
        ...
        <?php else: ?>
            ...
            <?php foreach ( $live as $p ): ?>
                <li style="flex-wrap:wrap;gap:8px">
                    ...
                    <span class="educbt-muted"><?php echo esc_html( wp_date( 'g:ia', strtotime( (string) $p['scheduled_at'] . ' UTC' ) ) ); ?></span>
                    <?php if ( ! empty( $p['access_code'] ) ) : ?>
                        <span style="font-size:13px;font-weight:700;letter-spacing:2px;background:#f0fdf4;padding:2px 10px;border-radius:4px;color:#166534"><?php echo esc_html( (string) $p['access_code'] ); ?></span>
                    <?php endif; ?>
