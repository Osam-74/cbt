...
$indiv_search  = sanitize_text_field( (string) ( $_GET['sq'] ?? '' ) );
$indiv_results = $indiv_search !== '' ? $promotion->search_students( $school_id, $indiv_search ) : [];

// All classes for the target dropdown
$all_classes = (array) $wpdb->get_results(
    $wpdb->prepare(
        'SELECT id, display_name, level_id FROM ' . \EduCBTPro\Core\Schema::table( 'classes' ) . " WHERE school_id = %d AND status = 'active' ORDER BY display_name ASC",
        $school_id
    ),
    ARRAY_A
);
...
$educbt_body = static function () use ( $flash, $sessions, $levels, $current, $review, $batches, $rules, $subject_options, $indiv_search, $indiv_results, $all_classes ): void {
    require EDUCBT_PRO_PATH . 'templates/portal/partials/flash.php';
    ?>
    <!-- Individual Promotion / Demotion -->
    <section class="educbt-card">
        <h2>Promote or demote a student individually</h2>
        <p class="educbt-muted" style="margin-top:-6px">
            Search by name or admission number. Move a student to any class without
            running a full batch — for late joiners, mid-year transfers, or corrections.
        </p>
        <form method="get" class="educbt-form" style="display:flex;gap:10px;align-items:flex-end;margin-top:8px">
            <div style="flex:1 1 auto">
                <label for="sq">Search student</label>
                <input id="sq" name="sq" type="text" value="<?php echo esc_attr( $indiv_search ); ?>" placeholder="Name or admission number">
            </div>
            <button class="educbt-btn" type="submit">Search</button>
        </form>

        <?php if ( $indiv_search !== '' ) : ?>
            <?php if ( empty( $indiv_results ) ) : ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
                                    <input type="hidden" name="action" value="educbt_move_student">
                                    <input type="hidden" name="student_id" value="<?php echo esc_attr( (string) $s['id'] ); ?>">
                                    <?php wp_nonce_field( 'educbt_move_student' ); ?>
                                    <select name="to_class_id" required style="padding:5px;min-width:140px">
                                        <option value="">Select class</option>
                                        <?php foreach ( $all_classes as $cl ) : ?>
                                            <option value="<?php echo esc_attr( (string) $cl['id'] ); ?>"><?php echo esc_html( (string) $cl['display_name'] ); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <select name="outcome" required style="padding:5px">
                                        <option value="promote">Promote</option>
                                        <option value="trial">Trial</option>
                                        <option value="repeat">Repeat</option>
                                        <option value="graduate">Graduate</option>
                                    </select>
                                    <input name="reason" type="text" placeholder="Reason" required style="padding:5px;min-width:100px">
                                    <button type="submit" class="educbt-btn educbt-btn--primary" style="padding:5px 14px">Move</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
        ...
        <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="educbt-form">
            ...
            <div class="educbt-grid">
                ...
                <div style="grid-column:1 / -1">
                    ...
                    <?php
                    echo \EduCBTPro\Frontend\ChipField::render( // phpcs:ignore WordPress.Security.EscapeOutput
                        'must_pass_codes[]',
                        $subject_options,
