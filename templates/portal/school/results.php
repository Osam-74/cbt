...
// 'review' is not a workflow state — it is a moderation screen the principal
// reaches from 'compiled'. It does not change the status; it lets them adjust
// scores before approving. That is why it is a separate button, not in next_step.
$next_step = [
...
$educbt_body = static function () use ( $flash, $classes, $status_by_class, $next_step, $session_id, $term_id, $session, $term, $enrolment_counts ): void {
                ...
                <?php foreach ( $classes as $class ) :
                    ...
                    <tr>
                        ...
                        <td style="display:flex;gap:8px;flex-wrap:wrap">
                            ...
                            <?php if ( $status === 'compiled' || $status === 'approved' ) : ?>
                                <a class="educbt-btn" href="<?php echo esc_url( home_url( '/portal/school/review/' . $cid ) ); ?>">Review &amp; Moderate</a>
                            <?php endif; ?>

                            <?php if ( isset( $next_step[ $status ] ) ) :
                                [ $to, $label ] = $next_step[ $status ]; ?>
                                <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="educbt-move-<?php echo esc_attr( (string) $cid ); ?>">
                                ...
                                <?php if ( $to === 'published' ) : ?>
                                    <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="educbt-force-<?php echo esc_attr( (string) $cid ); ?>" style="display:none">
                                        <input type="hidden" name="action" value="educbt_move_results">
                                        ...
                                        <input type="hidden" name="term_id" value="<?php echo esc_attr( (string) $term_id ); ?>">
                                        <?php wp_nonce_field( 'educbt_move_results' ); ?>
                                        <button type="submit" class="educbt-btn educbt-btn--primary" style="background:#dc2626">Publish Anyway</button>
                                    </form>
                                <?php endif; ?>
                            <?php endif; ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // If the flash error mentions "missing" or "Publish Anyway", reveal the
        // force-publish buttons so the principal can push through.
        var notes = document.querySelectorAll('.educbt-note--warn');
        notes.forEach(function(n) {
            var txt = n.textContent.toLowerCase();
            if (txt.indexOf('missing') !== -1 || txt.indexOf('publish anyway') !== -1) {
                document.querySelectorAll('[id^="educbt-force-"]').forEach(function(f) {
