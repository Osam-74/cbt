...
// Look up the staff record ID (educbt_staff.id) from the WordPress user ID.
// The signatures table stores educbt_staff.id, not wp_user_id — resolve_for_class()
// looks up signatures by staff record ID, so they must match.
...
// Fallback: if no staff record exists, use wp_user_id so at least the form works
if ( $staff_id === 0 ) {
    $staff_id = $wp_user_id;
}

// Detect if this user is an exam officer (has MANAGE_PAPERS but not MANAGE_SCHOOL)
$_is_exam_officer = \EduCBTPro\Core\Gate::allows( \EduCBTPro\Core\Capabilities::MANAGE_PAPERS )
    && ! \EduCBTPro\Core\Gate::allows( \EduCBTPro\Core\Capabilities::MANAGE_SCHOOL );
$_sig_role = $_is_exam_officer ? 'exam_officer' : 'class_teacher';
$_sig_context = $_is_exam_officer ? 'transcripts' : 'report sheets for students in your class';

// Look up existing signature using the CORRECT role
$existing = $sig_service->get_for_staff( $school_id, $staff_id, $_sig_role );

$default_name = $existing['name'] ?? ( $staff_name !== '' ? 'Mr./Mrs. ' . $staff_name : '' );

$educbt_title = 'My Signature';
$educbt_body = static function() use ( $existing, $staff_id, $default_name, $_sig_context, $_is_exam_officer, $_sig_role ): void {
    ...
    ?>
    <p class="educbt-help">Your signature appears on <?php echo esc_html( $_sig_context ); ?>. Draw, type, or upload below.</p>
    ...
    <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" enctype="multipart/form-data" class="educbt-form">
        ...
        <input type="hidden" name="signature_role" value="<?php echo esc_attr( $_sig_role ); ?>">
    ...
    (function() {
        ...
        document.getElementById('sig-type').addEventListener('change',function(){
            ...
            // Clear the data field when switching away from canvas mode
            if (v !== 'digital') { document.getElementById('sig-data').value = ''; }
        });
        // When submitting, ensure signature_data is set for the active mode
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                if (mode === 'text') {
                    ...
                    // Always capture canvas state on submit — mouseleave/mouseup
                    // might have missed it (e.g. user draws then clicks button)
                    var dataUrl = canvas.toDataURL();
                    // toDataURL returns a blank (white) image if nothing was
                    // drawn — check if canvas is actually non-empty
