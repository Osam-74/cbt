    ...
    $students = (array) $wpdb->get_results(
        $wpdb->prepare(
            "SELECT st.id, st.admission_number, st.first_name, st.last_name, st.full_name,
             ...
             WHERE e.school_id = %d AND e.class_id = %d AND e.session_id = %d AND e.status IN ('active', 'pending_approval')
               AND st.status IN ('active', 'pending_approval')
                    ...
                    <tbody>
                        <?php foreach ( $students as $i => $st ) : ?>
                            <tr>
                                ...
                                <td>
                                    <?php $st_status = (string) ( $st['status'] ?? 'active' ); ?>
