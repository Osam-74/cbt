...
// The WordPress display name is whatever the account was created with, and it goes
// stale the moment a school edits the person's real name in the portal — which is
// why a restored principal kept showing a previous school's name here. The staff
// record is the school's own copy and is what the rest of the portal shows, so
// prefer it and fall back to the WordPress name only when there is no staff row.
$actor        = ( new \EduCBTPro\Core\Scope() )->actor();
$display_name = trim( (string) $user->display_name );

// A principal often has NO staff row — Scope falls back to user meta and returns
// an actor with id 0 so they are not locked out of their own dashboard. That is
// the right call for access, but it left the name lookup below with nothing to
// look up, so the sidebar showed the WordPress account name, which for a
// principal account created at onboarding is the literal word "Principal".
//
// The school's own record of who the principal is lives on the school row, and is
// what the settings screen edits. Use it.
if ( ( $actor['type'] ?? '' ) === 'staff' && absint( $actor['id'] ?? 0 ) === 0 ) {
    // The school row is already loaded above; no second query needed.
    $principal_name = (string) ( $school_row['principal_name'] ?? '' );

    if ( trim( $principal_name ) !== '' ) {
        $display_name = trim( $principal_name );
    }
}
...
if ( ( $actor['type'] ?? '' ) === 'staff' && absint( $actor['id'] ?? 0 ) > 0 ) {
    $staff_name = (string) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT TRIM(CONCAT(first_name, ' ', last_name)) FROM " . \EduCBTPro\Core\Schema::table( 'staff' ) . '
    ...
    if ( trim( $staff_name ) !== '' ) {
        $display_name = trim( $staff_name );
    }
}

// A principal's staff row is created at onboarding with first_name = 'Principal'
// and last_name = ''. The school's principal_name setting is the authoritative
// source. Use it whenever it is set and non-empty.
if ( ( $actor['role'] ?? '' ) === \EduCBTPro\Core\Capabilities::ROLE_PRINCIPAL ) {
    $pn = trim( (string) ( $school_row['principal_name'] ?? '' ) );
    if ( $pn !== '' ) {
        $display_name = $pn;
    }
}

if ( ( $actor['type'] ?? '' ) === 'student' && absint( $actor['id'] ?? 0 ) > 0 ) {
    $student_name = (string) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT TRIM(CONCAT(first_name, ' ', last_name)) FROM " . $wpdb->prefix . 'educbt_students
             WHERE id = %d AND school_id = %d',
            absint( $actor['id'] ),
            $school_id
        )
    ...
    if ( trim( $student_name ) !== '' ) {
        $display_name = trim( $student_name );
    }
}

$name_parts = array_filter( explode( ' ', $display_name ) );
$initials   = '';
