...
class RestController {
    ...
    public function get_broadsheet( $request ): array {
        ...
        $type         = sanitize_text_field( $params['type']         ?? 'class' );
        $class        = sanitize_text_field( $params['class']        ?? '' );
        $subject      = sanitize_text_field( $params['subject']      ?? '' );
        $term         = sanitize_text_field( $params['term']         ?? '' );
        $session_year = sanitize_text_field( $params['session_year'] ?? '' );
        ...
        $data = match ( $type ) {
            'subject' => $svc->generate_subject_broadsheet( $school_id, $subject, $term, $session_year ),
            'session' => $svc->generate_session_broadsheet( $school_id, $class, $session_year ),
            default   => $svc->generate_class_broadsheet( $school_id, $class, $term, $session_year ),
        };
    ...
    public function get_promotions( $request ): array {
        ...
        $school_id = $this->tenant_context->get_school_id() ?? 0;
        return [ 'success' => true, 'data' => $svc->list_promotions( $school_id ) ];
    }
    ...
    public function create_promotion( $request ): array {
        ...
        $params = $request->get_json_params();
        $school_id = $this->tenant_context->get_school_id() ?? 0;
        if ( empty( $params['student_id'] ) || empty( $params['to_class'] ) ) {
            return [ 'success' => false, 'message' => 'student_and_to_class_required' ];
        }

        $id = $svc->create_promotion( $school_id, $params );
        return [ 'success' => (bool) $id, 'id' => $id ];
    }
    ...
    public function get_transcripts( $request ): array {
        ...
        $school_id = $this->tenant_context->get_school_id() ?? 0;
        return [ 'success' => true, 'data' => $svc->list_transcripts( $school_id ) ];
    }
    ...
    public function create_transcript( $request ): array {
        ...
        $id = $svc->create_transcript( $school_id, $params );
        return [ 'success' => (bool) $id, 'id' => $id ];
    }
    ...
    public function get_parent_transcripts( $request ): array {
        ...
        $svc = new TranscriptService();
        $school_id = $this->tenant_context->get_school_id() ?? 0;
        $transcripts = array_values(
            array_filter(
                $svc->list_transcripts( $school_id ),
                fn( $row ) => absint( $row['student_id'] ?? 0 ) === $student_id
            )
        );
    ...
    public function get_notifications( $request ): array {
        ...
        $data = $recipient_id > 0
            ? $svc->list_for_recipient( $school_id, $recipient_id )
            : $svc->list_notifications( $school_id );
    ...
    public function create_notification( $request ): array {
        ...
        $school_id = $this->tenant_context->get_school_id() ?? 0;
        $params = method_exists( $request, 'get_json_params' ) ? (array) $request->get_json_params() : [];
        return $svc->create_notification( $school_id, $params );
    }
    ...
    public function get_notifications_unread_count( $request ): array {
        ...
        return [ 'success' => true, 'count' => $svc->get_unread_count( $school_id, $recipient_id ) ];
    }
