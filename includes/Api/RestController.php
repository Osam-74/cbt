...
class RestController {
    ...
    public function start_student_exam( $request ): array {
        ...
        // Practice/CA test papers do not have a timetable entry — they are
        // always available while published. The student-side "sit a practice
        // paper" flow must work without a configured timetable.
        $is_practice_paper = false;
        if ( ! is_array( $timetable ) ) {
            $paper_check = $wpdb->get_var(
                $wpdb->prepare(
                    'SELECT is_practice FROM ' . \EduCBTPro\Core\Schema::table( 'exam_papers' ) . ' WHERE id = %d AND school_id = %d',
                    $exam_id, $school_id
                )
            );
            $is_practice_paper = ! empty( $paper_check );
            if ( ! $is_practice_paper ) {
        ...
        if ( is_array( $timetable ) ) {
            ...
            $is_trial = ! empty( $timetable['is_trial_mode'] ) || ! empty( $timetable['is_practice'] );
    ...
    public function get_student_exam_session( $request ): array {
        ...
        $timetable = $timetable_service->get_exam_timetable( $school_id, $exam_id );
        $is_trial_mode = is_array( $timetable ) && ( ! empty( $timetable['is_trial_mode'] ) || ! empty( $timetable['is_practice'] ) );
        // Also check the exam paper itself for is_practice flag — practice papers
        // may not have a timetable entry but still need immediate results.
        if ( ! $is_trial_mode ) {
            $paper_is_practice = $wpdb->get_var(
                $wpdb->prepare(
                    'SELECT is_practice FROM ' . \EduCBTPro\Core\Schema::table( 'exam_papers' ) . ' WHERE id = %d AND school_id = %d',
                    $exam_id, $school_id
                )
            );
            if ( ! empty( $paper_is_practice ) ) {
                $is_trial_mode = true;
            }
        }
