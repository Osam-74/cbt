...
class PlatformAdminController {
    ...
    public function render(): void {
                                ...
                                <tr id="sch-<?php echo esc_attr( (string) $sid ); ?>" style="display:none;background:#f6f7f7">
                                    <td colspan="5">
                                        <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="padding:10px 0">
                                            ...
                                            <p><label>Logo URL<br>
                                                <br>
                                                <?php // The create form has offered the media library since day one;
                                                      // the edit form asked for a URL, so re-attaching a crest meant
                                                      // going to the library, copying an address and pasting it back. ?>
                                                <input name="logo" type="hidden" class="educbt-edit-logo" value="<?php echo esc_attr( (string) ( $school['logo'] ?? '' ) ); ?>">
                                                <span class="educbt-edit-logo-preview">
                                                    <?php if ( ! empty( $school['logo'] ) ) : ?>
                                                        <img src="<?php echo esc_url( (string) $school['logo'] ); ?>" alt="" style="max-height:60px;display:block;margin-bottom:6px">
                                                    <?php endif; ?>
                                                </span>
                                                <button type="button" class="button educbt-edit-logo-pick">Choose logo</button>
                                                <button type="button" class="button-link educbt-edit-logo-clear" style="margin-left:8px">Remove</button>
                                                </label></p>
        ...
        jQuery(function ($) {
            var frame;
            // Delegated, because the edit forms are one per school row and are
            // rendered before this runs.
            $(document).on('click', '.educbt-edit-logo-pick', function (e) {
                e.preventDefault();
                var $wrap = $(this).closest('td');
                var frame = wp.media({ title: 'Choose school logo', multiple: false,
                                       library: { type: 'image' }, button: { text: 'Use this logo' } });
                frame.on('select', function () {
                    var att = frame.state().get('selection').first().toJSON();
                    $wrap.find('.educbt-edit-logo').val(att.url);
                    $wrap.find('.educbt-edit-logo-preview').html(
                        $('<img>').attr('src', att.url).css({ maxHeight: '60px', display: 'block', marginBottom: '6px' })
                    );
                });
                frame.open();
            });

            $(document).on('click', '.educbt-edit-logo-clear', function (e) {
                e.preventDefault();
                var $wrap = $(this).closest('td');
                $wrap.find('.educbt-edit-logo').val('');
                $wrap.find('.educbt-edit-logo-preview').empty();
            });
