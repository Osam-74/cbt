...
class AdminController {
    ...
    public function render_schools_page(): void {
                ...
                if ( ! empty( $_FILES['principal_photo_file']['name'] ) && isset( $_FILES['principal_photo_file']['error'] ) && $_FILES['principal_photo_file']['error'] === UPLOAD_ERR_OK ) {
                    ...
                    if ( false !== $check_image ) {
                        ...
                        if ( $movefile && ! isset( $movefile['error'] ) ) {
                            $school_data['principal_photo'] = (string) $movefile['url'];
                        }
                    }
                }
        ...
        if ( isset( $_POST['educbt_pro_action'] ) && 'update_school_admin' === sanitize_text_field( wp_unslash( $_POST['educbt_pro_action'] ) ) ) {
            if ( isset( $_POST['educbt_pro_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['educbt_pro_nonce'] ), 'educbt_pro_update_school_admin' ) ) {
                $edit_school_id = absint( wp_unslash( $_POST['school_id'] ?? 0 ) );
                if ( $edit_school_id > 0 ) {
                    $update_data = [
                        'school_name'    => sanitize_text_field( wp_unslash( $_POST['school_name'] ?? '' ) ),
                        'address'        => sanitize_textarea_field( wp_unslash( $_POST['address'] ?? '' ) ),
                    ...
                    if ( ! empty( $_FILES['principal_photo_file']['name'] ) && isset( $_FILES['principal_photo_file']['error'] ) && $_FILES['principal_photo_file']['error'] === UPLOAD_ERR_OK ) {
                        ...
                        if ( false !== $check_image ) {
                            $movefile = wp_handle_upload( $_FILES['principal_photo_file'], [ 'test_form' => false ] );
                            if ( $movefile && ! isset( $movefile['error'] ) ) {
                                $update_data['principal_photo'] = (string) $movefile['url'];
                            }
                        }
                    }

                    $this->school_service->update_school( $edit_school_id, $update_data );
                    $message = esc_html__( 'School updated successfully.', 'educbt-pro' );
                }
            }
        }
        ...
        echo '<tr><th scope="row"><label for="principal_photo_file">' . esc_html__( 'Principal Photo', 'educbt-pro' ) . '</label></th><td><input type="file" name="principal_photo_file" id="principal_photo_file" accept="image/*" /></td></tr>';
        echo '</table>';
        ...
        // Edit school form (for the currently selected school context).
        if ( $current_school_id > 0 ) {
            $edit_school = null;
            foreach ( $schools as $s ) {
                if ( (int) $s['id'] === $current_school_id ) { $edit_school = $s; break; }
            }
            if ( $edit_school ) {
                echo '<h2>' . esc_html__( 'Edit School', 'educbt-pro' ) . '</h2>';
                echo '<form method="post" action="" enctype="multipart/form-data">';
                wp_nonce_field( 'educbt_pro_update_school_admin', 'educbt_pro_nonce' );
                echo '<input type="hidden" name="educbt_pro_action" value="update_school_admin">';
                echo '<input type="hidden" name="school_id" value="' . esc_attr( (string) $current_school_id ) . '">';
                echo '<table class="form-table">';
                $this->render_input_row_with_value( 'school_name', __( 'School Name', 'educbt-pro' ), (string) ( $edit_school['school_name'] ?? '' ) );
                $this->render_input_row_with_value( 'phone', __( 'Phone', 'educbt-pro' ), (string) ( $edit_school['phone'] ?? '' ) );
                $this->render_input_row_with_value( 'email', __( 'Email', 'educbt-pro' ), (string) ( $edit_school['email'] ?? '' ) );
                $this->render_input_row_with_value( 'website', __( 'Website', 'educbt-pro' ), (string) ( $edit_school['website'] ?? '' ) );
                $this->render_textarea_row_with_value( 'address', __( 'Address', 'educbt-pro' ), (string) ( $edit_school['address'] ?? '' ) );
                $this->render_input_row_with_value( 'principal_name', __( 'Principal Name', 'educbt-pro' ), (string) ( $edit_school['principal_name'] ?? '' ) );
                echo '<tr><th scope="row"><label for="edit_principal_photo_file">' . esc_html__( 'Principal Photo', 'educbt-pro' ) . '</label></th>';
                echo '<td>';
                if ( ! empty( $edit_school['principal_photo'] ) ) {
                    echo '<div style="margin-bottom:8px"><img src="' . esc_url( (string) $edit_school['principal_photo'] ) . '" alt="" style="width:60px;height:72px;object-fit:cover;border-radius:6px;border:1px solid #ccc"> <span class="description">Current photo</span></div>';
                }
                echo '<input type="file" name="principal_photo_file" id="edit_principal_photo_file" accept="image/*" />';
                echo '</td></tr>';
                echo '</table>';
                submit_button( __( 'Update School', 'educbt-pro' ) );
                echo '</form>';
            }
        }
    ...
    public function render_promotions_page(): void {
        $message = '';
        // PromotionService moved to batch-based promotion in Phase 1: propose()
        // builds a batch for a whole class level, review()/override()/commit()
        // work it through. The per-student create_promotion()/list_promotions()
        // this screen called never existed, so opening this page was a fatal.
        // The working flow lives in the portal under Promotions.
        $message = esc_html__( 'Promotions are managed from the school portal, which runs the batch propose/review/commit flow. This screen is read-only.', 'educbt-pro' );
    ...
    public function render_transcripts_page(): void {
        ...
        // TranscriptService issues per student and keeps issuance history per
        // student. The create_transcript()/list_transcripts() pair this screen
        // called never existed, so opening the page was a fatal.
        if ( isset( $_POST['educbt_pro_action'] ) && 'create_transcript' === sanitize_text_field( wp_unslash( $_POST['educbt_pro_action'] ) ) ) {
            if ( isset( $_POST['educbt_pro_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['educbt_pro_nonce'] ), 'educbt_pro_add_transcript' ) ) {
                if ( $student_id > 0 ) {
                    $issued = $this->transcript_service->issue(
                        $this->get_current_school_id(),
                        $student_id,
                        absint( get_current_user_id() ),
                        sanitize_text_field( wp_unslash( $_POST['summary'] ?? '' ) )
                    );

                    $message = ! empty( $issued['serial'] )
                        ? esc_html__( 'Transcript issued successfully.', 'educbt-pro' )
                        : esc_html__( 'Unable to issue transcript.', 'educbt-pro' );
                } else {
                    $message = esc_html__( 'Select a student first.', 'educbt-pro' );
                }
            }
        }

        $transcripts = $student_id > 0
            ? $this->transcript_service->issuance_history( $this->get_current_school_id(), $student_id )
            : [];
    ...
    public function render_classes_page(): void {
        ...
        // Edit class
        if ( isset( $_POST['educbt_pro_action'] ) && 'edit_class' === sanitize_text_field( wp_unslash( $_POST['educbt_pro_action'] ) ) ) {
            if ( isset( $_POST['educbt_pro_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['educbt_pro_nonce'] ), 'educbt_pro_edit_class' ) ) {
                ...
                if ( $class_id > 0 && $current_school_id > 0 ) {
                    $updated = $this->class_service->update_class( $current_school_id, $class_id, $class_data );
                    $message = $updated
                        ? esc_html__( 'Class updated successfully.', 'educbt-pro' )
                        : esc_html__( 'Unable to update class.', 'educbt-pro' );
                }
            }
        }

        // Delete class
        if ( isset( $_POST['educbt_pro_action'] ) && 'delete_class' === sanitize_text_field( wp_unslash( $_POST['educbt_pro_action'] ) ) ) {
            if ( isset( $_POST['educbt_pro_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['educbt_pro_nonce'] ), 'educbt_pro_delete_class' ) ) {
                $class_id = absint( wp_unslash( $_POST['class_id'] ?? 0 ) );

                if ( $class_id > 0 && $current_school_id > 0 ) {
                    $deleted = $this->class_service->delete_class( $current_school_id, $class_id );
                    $message = $deleted
                        ? esc_html__( 'Class deleted successfully.', 'educbt-pro' )
                        : esc_html__( 'Unable to delete class.', 'educbt-pro' );
                }
            }
        }
        ...
        // Inline edit target
        $edit_class_id = absint( $_GET['edit_class'] ?? 0 );
        $edit_class = null;
        if ( $edit_class_id > 0 ) {
            foreach ( $classes as $cl ) {
                if ( (int) $cl['id'] === $edit_class_id ) {
                    $edit_class = $cl;
                    break;
                }
            }
        ...
        echo '<th>' . esc_html__( 'ID', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Class Name', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Arm', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Level', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Status', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Actions', 'educbt-pro' ) . '</th>';
        echo '</tr></thead><tbody>';

        if ( empty( $classes ) ) {
            echo '<tr><td colspan="6">' . esc_html__( 'No classes registered yet.', 'educbt-pro' ) . '</td></tr>';
        } else {
            foreach ( $classes as $class ) {
                // Inline edit row
                if ( $edit_class && (int) $class['id'] === $edit_class_id ) {
                    echo '<tr style="background:#fffde7">';
                    echo '<form method="post" action="">';
                    wp_nonce_field( 'educbt_pro_edit_class', 'educbt_pro_nonce' );
                    echo '<input type="hidden" name="educbt_pro_action" value="edit_class">';
                    echo '<input type="hidden" name="class_id" value="' . esc_attr( $class['id'] ) . '">';
                    echo '<td>' . esc_html( $class['id'] ) . '</td>';
                    echo '<td><input type="text" name="class_name" value="' . esc_attr( $class['class_name'] ) . '" style="width:100%"></td>';
                    echo '<td><input type="text" name="arm" value="' . esc_attr( $class['arm'] ) . '" style="width:50px"></td>';
                    echo '<td><input type="text" name="class_level" value="' . esc_attr( $class['class_level'] ) . '" style="width:80px"></td>';
                    echo '<td><input type="text" name="status" value="' . esc_attr( $class['status'] ) . '" style="width:80px"></td>';
                    echo '<td><button type="submit" class="button button-primary">Save</button> ';
                    echo '<a href="' . esc_url( remove_query_arg( 'edit_class' ) ) . '" class="button">Cancel</a></td>';
                    echo '</form>';
                    echo '</tr>';
                    continue;
                }
                ...
                echo '<a href="' . esc_url( add_query_arg( 'edit_class', $class['id'] ) ) . '" class="button button-small">Edit</a> ';
                echo '<form method="post" action="" style="display:inline" onsubmit="return confirm(\'Delete this class?\')">';
                wp_nonce_field( 'educbt_pro_delete_class', 'educbt_pro_nonce' );
                echo '<input type="hidden" name="educbt_pro_action" value="delete_class">';
                echo '<input type="hidden" name="class_id" value="' . esc_attr( $class['id'] ) . '">';
    ...
    public function render_subjects_page(): void {
        ...
        // Edit subject
        if ( isset( $_POST['educbt_pro_action'] ) && 'edit_subject' === sanitize_text_field( wp_unslash( $_POST['educbt_pro_action'] ) ) ) {
            if ( isset( $_POST['educbt_pro_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['educbt_pro_nonce'] ), 'educbt_pro_edit_subject' ) ) {
                $subject_id = absint( wp_unslash( $_POST['subject_id'] ?? 0 ) );
                $subject_data = [
                ...
                if ( $subject_id > 0 && $current_school_id > 0 ) {
                    $updated = $this->subject_service->update_subject( $current_school_id, $subject_id, $subject_data );
                    $message = $updated
                        ? esc_html__( 'Subject updated successfully.', 'educbt-pro' )
                        : esc_html__( 'Unable to update subject.', 'educbt-pro' );
                }
            }
        }

        // Delete subject
        if ( isset( $_POST['educbt_pro_action'] ) && 'delete_subject' === sanitize_text_field( wp_unslash( $_POST['educbt_pro_action'] ) ) ) {
            if ( isset( $_POST['educbt_pro_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['educbt_pro_nonce'] ), 'educbt_pro_delete_subject' ) ) {
                $subject_id = absint( wp_unslash( $_POST['subject_id'] ?? 0 ) );

                if ( $subject_id > 0 && $current_school_id > 0 ) {
                    $result = $this->subject_service->delete_subject( $current_school_id, $subject_id );
                    if ( $result['success'] ) {
                        $message = $result['references'] > 0
                            ? sprintf( esc_html__( 'Subject deleted. %d questions referenced it (they still hold the old name).', 'educbt-pro' ), $result['references'] )
                            : esc_html__( 'Subject deleted successfully.', 'educbt-pro' );
                    } else {
                        $message = esc_html__( 'Unable to delete subject.', 'educbt-pro' );
                    }
                }
            }
        }
        ...
        if ( isset( $_POST['educbt_pro_action'] ) && 'merge_subject' === sanitize_text_field( wp_unslash( $_POST['educbt_pro_action'] ) ) ) {
            if ( isset( $_POST['educbt_pro_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['educbt_pro_nonce'] ), 'educbt_pro_merge_subject' ) ) {
                $source_id = absint( wp_unslash( $_POST['source_id'] ?? 0 ) );
                $target_id = absint( wp_unslash( $_POST['target_id'] ?? 0 ) );

                if ( $source_id > 0 && $target_id > 0 && $current_school_id > 0 ) {
                    $merged = $this->subject_service->merge_subjects( $current_school_id, $source_id, $target_id );
                    $message = $merged
                        ? esc_html__( 'Subjects merged successfully. All questions now reference the target subject.', 'educbt-pro' )
                        : esc_html__( 'Unable to merge subjects.', 'educbt-pro' );
                }
            }
        }
        ...
        // Detect duplicates (same name, different case/spelling)
        $name_groups = [];
        foreach ( $subjects as $s ) {
            $key = strtolower( trim( $s['subject_name'] ) );
        ...
        if ( ! empty( $duplicates ) ) {
            echo '<div class="notice notice-warning"><p><strong>' . esc_html__( 'Duplicate subjects detected.', 'educbt-pro' ) . '</strong> ' . esc_html__( 'Use the Merge dropdown to consolidate them.', 'educbt-pro' ) . '</p></div>';
        }

        // Inline edit target
        $edit_id = absint( $_GET['edit_subject'] ?? 0 );
        $edit_subject = null;
        if ( $edit_id > 0 ) {
            foreach ( $subjects as $s ) {
                if ( (int) $s['id'] === $edit_id ) {
        ...
        echo '<table class="widefat fixed striped"><thead><tr>';
        echo '<th>' . esc_html__( 'ID', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Name', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Code', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Type', 'educbt-pro' ) . '</th>';
        echo '<th>' . esc_html__( 'Actions', 'educbt-pro' ) . '</th>';
        echo '</tr></thead><tbody>';

        if ( empty( $subjects ) ) {
            echo '<tr><td colspan="5">' . esc_html__( 'No subjects registered yet.', 'educbt-pro' ) . '</td></tr>';
        } else {
            foreach ( $subjects as $subject ) {
                // Inline edit row
                if ( $edit_subject && (int) $subject['id'] === $edit_id ) {
                    echo '<tr style="background:#fffde7">';
                    echo '<form method="post" action="">';
                    wp_nonce_field( 'educbt_pro_edit_subject', 'educbt_pro_nonce' );
                    echo '<input type="hidden" name="educbt_pro_action" value="edit_subject">';
                    echo '<input type="hidden" name="subject_id" value="' . esc_attr( $subject['id'] ) . '">';
                    echo '<td>' . esc_html( $subject['id'] ) . '</td>';
                    echo '<td><input type="text" name="subject_name" value="' . esc_attr( $subject['subject_name'] ) . '" style="width:100%"></td>';
                    echo '<td><input type="text" name="subject_code" value="' . esc_attr( $subject['subject_code'] ) . '" style="width:80px"></td>';
                    echo '<td><input type="text" name="subject_type" value="' . esc_attr( $subject['subject_type'] ) . '" style="width:80px"></td>';
                    echo '<td><button type="submit" class="button button-primary">Save</button> ';
                    echo '<a href="' . esc_url( remove_query_arg( 'edit_subject' ) ) . '" class="button">Cancel</a></td>';
                    echo '</form>';
                    echo '</tr>';
                    continue;
                }
                ...
                echo '<tr>';
                echo '<td>' . esc_html( $subject['id'] ) . '</td>';
                echo '<td>' . esc_html( $subject['subject_name'] );
                // Highlight duplicates
                $key = strtolower( trim( $subject['subject_name'] ) );
                if ( isset( $duplicates[ $key ] ) && count( $duplicates[ $key ] ) > 1 ) {
                    echo ' <span style="color:#d97706;font-size:.8rem">⚠ duplicate</span>';
                }
                echo '</td>';
                ...
                echo '<td style="white-space:nowrap">';
                // Edit
                echo '<a href="' . esc_url( add_query_arg( 'edit_subject', $subject['id'] ) ) . '" class="button button-small">Edit</a> ';
                // Delete
                echo '<form method="post" action="" style="display:inline" onsubmit="return confirm(\'Delete this subject? Questions referencing it will keep the old name.\')">';
                wp_nonce_field( 'educbt_pro_delete_subject', 'educbt_pro_nonce' );
                echo '<input type="hidden" name="educbt_pro_action" value="delete_subject">';
                echo '<input type="hidden" name="subject_id" value="' . esc_attr( $subject['id'] ) . '">';
                echo '<button type="submit" class="button button-small button-link-delete">Delete</button>';
                echo '</form>';
                echo '</td>';
        ...
        // Merge UI (only if duplicates exist)
        if ( ! empty( $duplicates ) ) {
            echo '<h3>' . esc_html__( 'Merge Duplicate Subjects', 'educbt-pro' ) . '</h3>';
            echo '<p>' . esc_html__( 'Select a duplicate to merge into a target. All questions referencing the source will be updated to the target name.', 'educbt-pro' ) . '</p>';
            echo '<form method="post" action="" style="margin:12px 0 20px;">';
            wp_nonce_field( 'educbt_pro_merge_subject', 'educbt_pro_nonce' );
            echo '<input type="hidden" name="educbt_pro_action" value="merge_subject">';
            echo '<table class="form-table">';
            echo '<tr><th>' . esc_html__( 'Source (will be deleted)', 'educbt-pro' ) . '</th><td>';
            echo '<select name="source_id">';
            foreach ( $subjects as $s ) {
                $key = strtolower( trim( $s['subject_name'] ) );
                if ( isset( $duplicates[ $key ] ) && count( $duplicates[ $key ] ) > 1 ) {
                    echo '<option value="' . esc_attr( $s['id'] ) . '">' . esc_html( $s['subject_name'] . ' (' . $s['subject_code'] . ')' ) . '</option>';
                }
            }
            echo '</select></td></tr>';
            echo '<tr><th>' . esc_html__( 'Target (will keep)', 'educbt-pro' ) . '</th><td>';
            ...
            echo '</select></td></tr>';
            echo '</table>';
            submit_button( __( 'Merge Subjects', 'educbt-pro' ), 'primary' );
            echo '</form>';
        }
        ...
        if ( isset( $_POST['educbt_pro_action'] ) && 'verify_question_submissions' === sanitize_text_field( wp_unslash( $_POST['educbt_pro_action'] ) ) ) {
            if ( isset( $_POST['educbt_pro_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['educbt_pro_nonce'] ), 'educbt_pro_verify_question_submissions' ) ) {
                ...
                foreach ( $target_subjects as $subject_name ) {
                    ...
                    $pending_alerts = $teacher_id > 0 ? $this->notification_service->unread_count( $current_school_id, $teacher_id ) : 0;
    ...
    private function notify_teachers_for_pending_subjects( int $school_id, array $report_rows, string $term, string $session_year, string $class_name, string $department ): int {
        ...
        foreach ( $pending_rows as $row ) {
            ...
            // NotificationService::notify() is the real entry point; the
            // create_notification() array form called here never existed, which
            // made this whole alerting pass a fatal.
            $notification_id = $this->notification_service->notify(
                $school_id,
                $teacher_id,
                'alert',
                sprintf( 'Question Submission Pending - %s %s', $term, $session_year ),
                sprintf(
    ...
    private function should_send_pending_notification( int $school_id, int $teacher_id, string $signature, int $remaining ): bool {
        $notifications = $this->notification_service->inbox( $school_id, $teacher_id, false, 200 );
