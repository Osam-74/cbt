...
class ExamClock {
    ...
    /**
     * Convert a stored MySQL datetime to a UTC timestamp.
     *
     * All datetimes in this system are stored as UTC via current_time('mysql', true)
     * or gmdate(). The previous implementation called get_gmt_from_date() which
     * treated the already-UTC value as site-local time and subtracted the timezone
     * offset — inflating elapsed_seconds by the offset (e.g., +3600s for UTC+1).
     * That caused short CA tests to expire immediately on start.
     */
