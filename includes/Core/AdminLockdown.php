...
class AdminLockdown {
    ...
    private function is_portal_endpoint(): bool {
        ...
        // Fallback: some server configurations (nginx, LiteSpeed, CGI/FastCGI) do
        // not set SCRIPT_NAME the same way Apache does.  When that happens,
        // admin-post.php is no longer recognised, the lockdown kicks in, and
        // every portal form POST is silently redirected to the dashboard before
        // the handler ever runs — "the user pressed Save, landed back on their
        // dashboard, and nothing was written."
        //
        // If we are inside admin_init and a POST carries an 'action' parameter
        // that belongs to this plugin, it is a portal form submission to
        // admin-post.php — let it through.
        ...
        // Also check PHP_SELF and REQUEST_URI as fallbacks for SCRIPT_NAME.
        foreach ( [ 'PHP_SELF', 'REQUEST_URI' ] as $key ) {
            if ( ! empty( $_SERVER[ $key ] ) ) {
                $val = basename( (string) wp_unslash( $_SERVER[ $key ] ) );
                if ( in_array( $val, [ 'admin-post.php', 'admin-ajax.php' ], true ) ) {
                    return true;
                }
            }
        }
