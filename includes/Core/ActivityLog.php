...
class ActivityLog {
    ...
    private static function subscriptions(): array {
        return [
            ...
            // The single most common thing a teacher does was missing from the feed
            // entirely: submitting a question set for review, and the reviewer's
            // decision on it. The principal's "recent activity" looked broken
            // because the busiest part of the term left no trace.
            'educbt_question_set_submitted' => static fn( array $p ): string =>
                sprintf(
                    'Submitted %d %s question(s) for review — %s',
                    absint( $p['count'] ?? 0 ),
                    (string) ( $p['exam_type'] ?? '' ),
                    (string) ( $p['subject_name'] ?? '' )
                ),

            'educbt_questions_reviewed' => static function ( array $p ): string {
                $decision = (string) ( $p['decision'] ?? '' );
                $label    = $decision === 'approved' ? 'Approved' : ( $decision === 'revision' ? 'Sent back for revision' : 'Reviewed' );

                return sprintf( '%s %d question(s)', $label, absint( $p['changed'] ?? 0 ) );
            },

            'educbt_question_set_withdrawn' => static fn( array $p ): string =>
                sprintf( 'Withdrew a question set — %s', (string) ( $p['subject_name'] ?? '' ) ),

            'educbt_class_created' => static fn( array $p ): string =>
                sprintf( 'Created class %s', (string) ( $p['display_name'] ?? '' ) ),
