...
class Schema {
    ...
    public static function statements( string $collate ): array {
        ...
        $sql[] = "CREATE TABLE {$t('exam_papers')} (
            ...
            delivery_mode varchar(10) NOT NULL DEFAULT 'cbt',
            objective_per_student int(11) NOT NULL DEFAULT 0,
            status varchar(20) NOT NULL DEFAULT 'draft',
        ...
        $sql[] = "CREATE TABLE {$t('paper_invigilators')} (
            ...
            created_at datetime DEFAULT CURRENT_TIMESTAMP,
            PRIMARY KEY  (id),
            UNIQUE KEY paper_unique (paper_id),
            KEY school_id (school_id),
        ...
        $sql[] = "CREATE TABLE {$t('question_sets')} (
            ...
            term_id bigint(20) unsigned DEFAULT NULL,
            subject_id bigint(20) unsigned NOT NULL,
            -- A question set belongs to a LEVEL, not to an arm. JS1 A and JS1 B sit
            -- the same paper, so keying on class_id forced a teacher to enter the
            -- same questions once per arm. class_id is retained only so older rows
            -- and paper generation can still resolve, and is no longer part of the
            -- identity of a set.
            ...
            updated_at datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            PRIMARY KEY  (id),
            UNIQUE KEY scope (school_id, session_id, term_id, subject_id, level_id, department_id, exam_type, series_id),
        ...
        // ---------------------------------------------------------------
        // Signatures — digital or uploaded, for report sheets
        // ---------------------------------------------------------------
        $sql[] = "CREATE TABLE {$t('signatures')} (
            ...
            signature_data longtext NOT NULL DEFAULT '',
            display_name varchar(150) NOT NULL DEFAULT '',
            created_at datetime DEFAULT CURRENT_TIMESTAMP,
            updated_at datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            PRIMARY KEY  (id),
            UNIQUE KEY school_staff (school_id,staff_id,role),
            KEY school_id (school_id)
        ) {$collate};";
        ...
        // ---------------------------------------------------------------
        $sql[] = "CREATE TABLE {$t('remark_ranges')} (
            id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
            school_id bigint(20) unsigned NOT NULL,
            role varchar(30) NOT NULL DEFAULT 'class_teacher',
            min_avg decimal(5,2) NOT NULL DEFAULT 0,
            max_avg decimal(5,2) NOT NULL DEFAULT 100,
            ...
            updated_at datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            PRIMARY KEY  (id),
            KEY school_role (school_id,role),
            KEY school_id (school_id)
        ) {$collate};";
    ...
    public static function table_names(): array {
        return [
            ...
            'migration_map', 'signatures', 'remark_ranges',
        ];
    }
}
