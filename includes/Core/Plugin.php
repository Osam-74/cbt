...
class Plugin {
    ...
    public function init(): void {
        ...
        // Auto-close practice tests whose close time has passed.
        // Runs on every init so the close happens without needing a separate cron.
        try {
            ( new \EduCBTPro\Services\ExamPaperService() )->maybe_auto_close();
        } catch ( \Throwable $e ) {
            if ( function_exists( 'error_log' ) ) {
                error_log( 'EduCBT: auto-close failed: ' . $e->getMessage() );
            }
        }
    ...
    /** Phases 0-4 applied. */
    public const PHASE_VERSION = '12.2.4';
    ...
    public static function enqueue_portal_assets(): void {
        ...
        // Print rules load on every portal page, not just the ones with a Print
        // button: a user can always hit Ctrl+P, and without this the browser
        // reproduces the whole interface — navigation, cards and all — which is why
        // printed output looked like a screenshot rather than a document.
        $print_relative = 'assets/css/educbt-print.css';
        $print_path     = EDUCBT_PRO_DIR . $print_relative;

        if ( file_exists( $print_path ) ) {
            wp_enqueue_style(
                'educbt-print',
                EDUCBT_PRO_URL . $print_relative,
                [ 'educbt-portal' ],
                (string) filemtime( $print_path )
            );
        }
    ...
    public static function maybe_upgrade(): void {
        ...
        // Two independent version counters guard this: PHASE_VERSION here, and the
        // migration registry's own stored version. Adding a migration while leaving
        // PHASE_VERSION untouched used to mean it NEVER ran — the release shipped,
        // the option already matched, and the ALTER TABLE silently never happened.
        // Asking the registry directly removes the need to remember two bumps.
        $pending = false;

        try {
            $manager = new MigrationManager();
            Migrations::register( $manager );
            $pending = $manager->has_pending();
        } catch ( \Throwable $e ) {
            $pending = false;
        }

        if ( $stored === self::PHASE_VERSION && ! $pending ) {
            return;
        }
    ...
    private static function run_activation(): void {
        ...
        // Clean up legacy version-specific demo data flags so they don't
        // accumulate in the options table across upgrades.
        $wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'educbt_pro_demo_data_seeded_%'" );
    ...
    public static function on_uninstall(): void {
        ...
        // A SCHOOL'S DATA IS NOT DELETED HERE, AND THIS IS DELIBERATE.
        //
        // WordPress runs the uninstall hook when a plugin is DELETED from the
        // Plugins screen — which is exactly what an administrator does to install
        // a newer copy by hand. The previous version of this method dropped the
        // schools, users, students and questions tables at that moment, so a
        // routine manual update destroyed every student record and every school
        // row in the installation. Staff and results tables were not on the list,
        // so what survived pointed at schools and students that no longer existed:
        // the school vanished from the admin list, logins still worked, and every
        // screen came up empty.
        //
        // Losing a term of examination records must never be a side effect of
        // reinstalling software. Data is removed only when somebody asks for it
        // explicitly, by setting the option below and then uninstalling.
        $purge = get_option( 'educbt_pro_purge_on_uninstall', '' );

        if ( $purge !== 'yes-delete-everything' ) {
            // Leave every table exactly as it is. A fresh install finds the data
            // waiting and simply reconnects to it.
            delete_option( 'educbt_pro_bootstrap_complete' );
            return;
        }

        foreach ( (array) \EduCBTPro\Core\Schema::table_names() as $table ) {
        ...
        // Delete ALL demo data flags (both version-specific legacy and the new single flag)
    ...
    private static function seed_demo_data(): void {
        // Use a single version-independent flag so upgrading the plugin never
        // re-creates the demo school. Previous versions used a version-specific
        // flag ('educbt_pro_demo_data_seeded_' . VERSION) which meant every
        // version bump would seed a fresh demo school on the next activation —
        // exactly the "demo school keeps coming back" bug.
        $demo_flag = 'educbt_pro_demo_data_seeded';
        ...
        // If the install already has real schools, never seed demo data.
        // This protects production installs from accidentally getting a demo
        // school injected on re-activation.
        $schools_table = $wpdb->prefix . 'educbt_schools';
        $existing_schools = (int) $wpdb->get_var(
            "SELECT COUNT(*) FROM {$schools_table} WHERE school_name NOT LIKE 'Demo %' AND school_code NOT LIKE 'DSS%'"
        );
        ...
        update_option( 'educbt_pro_demo_data_seeded', '1' );
    }
    ...
    private static function maybe_upgrade_schema(): void {
        ...
        if ( version_compare( $schema_version, self::SCHEMA_VERSION, '<' ) ) {
            ...
            // v3.0.2: add closes_at column to exam_papers for test close-time support
            $papers_table = \EduCBTPro\Core\Schema::table( 'exam_papers' );
            $papers_cols  = (array) $wpdb->get_col( "SHOW COLUMNS FROM {$papers_table} LIKE 'closes_at'" );
            if ( empty( $papers_cols ) ) {
                $wpdb->query( "ALTER TABLE {$papers_table} ADD COLUMN closes_at datetime DEFAULT NULL AFTER scheduled_at" );
            }

            // v3.0.3: add delivery_mode column to exam_papers and question_sets
            // for CBT vs Written (paper-based) exam delivery mode.
            $papers_dm = (array) $wpdb->get_col( "SHOW COLUMNS FROM {$papers_table} LIKE 'delivery_mode'" );
            if ( empty( $papers_dm ) ) {
                $wpdb->query( "ALTER TABLE {$papers_table} ADD COLUMN delivery_mode varchar(10) NOT NULL DEFAULT 'cbt' AFTER is_practice" );
            }
            $sets_table = \EduCBTPro\Core\Schema::table( 'question_sets' );
            $sets_dm = (array) $wpdb->get_col( "SHOW COLUMNS FROM {$sets_table} LIKE 'delivery_mode'" );
            if ( empty( $sets_dm ) ) {
                $wpdb->query( "ALTER TABLE {$sets_table} ADD COLUMN delivery_mode varchar(10) NOT NULL DEFAULT 'cbt' AFTER exam_type" );
            }

            // v3.1.0: add objective_per_student column to exam_papers.
            // Stores how many objective questions each student sees, separate
            // from theory count. Used by compose() to size the per-student draw.
            $ops_cols = (array) $wpdb->get_col( "SHOW COLUMNS FROM {$papers_table} LIKE 'objective_per_student'" );
            if ( empty( $ops_cols ) ) {
                $wpdb->query( "ALTER TABLE {$papers_table} ADD COLUMN objective_per_student int(11) NOT NULL DEFAULT 0 AFTER delivery_mode" );
            }
    ...
    public function handle_student_admin_access(): void {
        ...
        // admin-post.php is how the portal submits forms (subject registration,
        // profile updates, etc). Blocking it here means every student form
        // POST is silently redirected to the dashboard and nothing is saved.
        // The same exemption exists in AdminLockdown::is_portal_endpoint().
        $script = isset( $_SERVER['SCRIPT_NAME'] ) ? basename( (string) wp_unslash( $_SERVER['SCRIPT_NAME'] ) ) : '';
        if ( $script === 'admin-post.php' ) {
            return;
        }

        // Fallback for servers where SCRIPT_NAME is not reliable (nginx,
        // LiteSpeed, CGI). If a POST carries an educbt_ action, it is one
        // of our form submissions — let it through.
        if ( ! empty( $_POST['action'] ) ) {
            $action = (string) wp_unslash( $_POST['action'] );
            if ( strpos( $action, 'educbt_' ) === 0 ) {
                return;
            }
        }

        // Also check PHP_SELF and REQUEST_URI as fallbacks.
        foreach ( [ 'PHP_SELF', 'REQUEST_URI' ] as $key ) {
            if ( ! empty( $_SERVER[ $key ] ) ) {
                $val = basename( (string) wp_unslash( $_SERVER[ $key ] ) );
                if ( $val === 'admin-post.php' ) {
                    return;
                }
            }
        }
