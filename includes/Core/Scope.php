...
class Scope {
    ...
    public function actor(): array {
        ...
        $student_row = $wpdb->get_row(
            $wpdb->prepare( "SELECT id, status FROM {$students} WHERE wp_user_id = %d LIMIT 1", $user_id ),
            ARRAY_A
        );
        $student_id = $student_row['id'] ?? null;
        $student_status = (string) ( $student_row['status'] ?? '' );

        // Fallback: a student may have been created in v1 without a wp_user_id
        // link. Try to match by email or login_username.
        if ( ! $student_id ) {
            ...
            if ( $user_email !== '' ) {
                $row = $wpdb->get_row(
                    $wpdb->prepare( "SELECT id, status FROM {$students} WHERE parent_email = %s LIMIT 1", $user_email ),
                    ARRAY_A
                );
                $student_id = $row['id'] ?? null;
                $student_status = (string) ( $row['status'] ?? '' );
            }
            if ( ! $student_id && $user_login !== '' ) {
                $row = $wpdb->get_row(
                    $wpdb->prepare( "SELECT id, status FROM {$students} WHERE login_username = %s LIMIT 1", $user_login ),
                    ARRAY_A
                );
                $student_id = $row['id'] ?? null;
                $student_status = (string) ( $row['status'] ?? '' );
            }
            // If we found them, backfill the wp_user_id so future lookups are instant.
            if ( $student_id ) {
                $wpdb->update(
                    $students,
                    [ 'wp_user_id' => $user_id ],
        ...
        if ( $student_id ) {
            // Inactive or withdrawn students are blocked from the portal.
            // They see a clear message instead of their dashboard.
            if ( $student_status === 'inactive' || $student_status === 'withdrawn' ) {
                return $this->actor = [
                    'type'      => self::ACTOR_NONE,
                    'id'        => 0,
                    'school_id' => $school_id,
                    'role'      => '',
                ];
