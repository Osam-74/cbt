...
class GradingWorker {
    ...
    public function grade( int $school_id, int $attempt_id ): array {
        ...
        // An unanswered OBJECTIVE question is worth nothing, but it must be recorded
        // as wrong rather than left NULL, or "how many did they attempt" becomes
        // unanswerable.
        //
        // This must NOT touch theory answers. A theory answer always has
        // option_id NULL — it is stored as answer_text, not a chosen option — so
        // this step used to zero out and "close" every theory answer the instant
        // the student submitted, before any teacher had marked it. That made the
        // marking queue think every theory answer was already marked (is_correct
        // was no longer NULL) and silently gave the theory section a permanent
        // score of zero. The question_type join below is what tells "no option
        // chosen" apart from "not an option-based question at all".
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$answers} a
                 ...
                 SET a.is_correct = 0, a.marks_awarded = 0
                 WHERE a.attempt_id = %d AND a.school_id = %d
                   AND a.option_id IS NULL
                   AND q.question_type <> %s",
                $attempt_id,
                $school_id,
                \EduCBTPro\Services\QuestionBankService::TYPE_THEORY
            )
        );
        ...
        //
        // For theory questions with children (1A, 1B, 1C…), only the parent is in
        // paper_questions. The student answers the CHILDREN, not the parent, so
        // the achievable marks are the sum of children's marks — not the parent's.
        // Using the parent's marks would either inflate the denominator (parent has
        // more marks than the children combined) or deflate it (parent has 0). We
        // substitute the children's total when a parent has children.
        $questions_t = $wpdb->prefix . 'educbt_questions';
