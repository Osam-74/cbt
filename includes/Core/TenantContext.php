...
class TenantContext {
    ...
    private function resolve_from_logged_in_user(): ?int {
        ...
        // Fallback: a student created in v1 may not have wp_user_id set. Try
        // matching by email or login_username.
        if ( ! $school_id ) {
            $user_info  = get_userdata( $user_id );
            $user_email = (string) ( $user_info->user_email ?? '' );
            $user_login = (string) ( $user_info->user_login ?? '' );

            if ( $user_email !== '' ) {
                $school_id = $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT school_id FROM {$students_table} WHERE parent_email = %s LIMIT 1",
                        $user_email
                    )
                );
            }
            if ( ! $school_id && $user_login !== '' ) {
                $school_id = $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT school_id FROM {$students_table} WHERE login_username = %s LIMIT 1",
                        $user_login
                    )
                );
            }
            // Backfill wp_user_id so future lookups are instant.
            if ( $school_id ) {
                $wpdb->update(
                    $students_table,
                    [ 'wp_user_id' => $user_id ],
    ...
    public function create_tables(): void {
        ...
        foreach ( [
            ...
            'principal_photo' => "ALTER TABLE {$schools_table} ADD COLUMN principal_photo varchar(255) NOT NULL DEFAULT ''",
...
$this->ensure_column( $students_table, 'registration_number', "ALTER TABLE {$students_table} ADD COLUMN registration_number varchar(100) NOT NULL DEFAULT '' AFTER admission_number" );
        ...
        // When the student's standing last changed. An expulsion has a 30-day
        // window in which the school may reconsider, which cannot be worked out
        // without a date.
        $this->ensure_column( $students_table, 'status_changed_at', "ALTER TABLE {$students_table} ADD COLUMN status_changed_at datetime DEFAULT NULL" );
        ...
        //
        // A CA test is an examination in every mechanical sense — it has a window,
        // a timetable, papers and attempts — so it reuses exam_series rather than
        // getting a parallel set of tables that would drift out of step. What makes
        // it a CA test rather than a terminal examination is series_type, plus the
        // assessment component its marks land in.
        $series_table = \EduCBTPro\Core\Schema::table( 'exam_series' );

        $this->ensure_column( $series_table, 'component_id', "ALTER TABLE {$series_table} ADD COLUMN component_id bigint(20) unsigned DEFAULT NULL" );
        ...
        // Integrity incidents are counted separately from question bookmarks.
        // Both used to increment flag_count, so an invigilator could not tell a
        // student who bookmarked four questions from one who left the exam window
        // four times — the column showed "4" either way.
        $attempts_table = \EduCBTPro\Core\Schema::table( 'attempts' );
        $this->ensure_column( $attempts_table, 'integrity_count', "ALTER TABLE {$attempts_table} ADD COLUMN integrity_count int(11) NOT NULL DEFAULT 0" );

        $sets_table = \EduCBTPro\Core\Schema::table( 'question_sets' );
        $this->ensure_column( $sets_table, 'series_id', "ALTER TABLE {$sets_table} ADD COLUMN series_id bigint(20) unsigned DEFAULT NULL" );

        // The UNIQUE KEY 'scope' on question_sets did not originally include
        // series_id, so practice/CA test sets for a subject that already had a
        // terminal examination set in the same term collided with a duplicate
        // key error. Drop and recreate the index to include series_id.
        $existing_keys = $wpdb->get_col( $wpdb->prepare(
            'SELECT DISTINCT index_name FROM information_schema.statistics WHERE table_schema = %s AND table_name = %s AND index_name = %s',
            DB_NAME, $sets_table, 'scope'
        ) );
        if ( ! empty( $existing_keys ) ) {
            // Check if series_id is already in the index — if so, skip.
            $index_cols = $wpdb->get_col( $wpdb->prepare(
            ...
            if ( ! in_array( 'series_id', $index_cols, true ) ) {
                $wpdb->query( "ALTER TABLE {$sets_table} DROP INDEX scope" );
                $wpdb->query( "ALTER TABLE {$sets_table} ADD UNIQUE KEY scope (school_id, session_id, term_id, subject_id, level_id, department_id, exam_type, series_id)" );
            }
        }

        // These identify a set alongside subject, level and term, so a query that
        // references a missing one fails outright — the set is never found, creating
        // it fails too, and the teacher is told "could not start a question set".
        //
        // They existed only as a migration, and a migration that has recorded itself
        // as run will not re-apply after a table is rebuilt. Guaranteed here instead,
        // where the check is against the table rather than a version number.
        $this->ensure_column( $sets_table, 'waec_mode', "ALTER TABLE {$sets_table} ADD COLUMN waec_mode tinyint(1) NOT NULL DEFAULT 0" );
        $this->ensure_column( $sets_table, 'delivery_mode', "ALTER TABLE {$sets_table} ADD COLUMN delivery_mode varchar(20) NOT NULL DEFAULT 'cbt'" );
        $this->ensure_column( $sets_table, 'reviewer_comment', "ALTER TABLE {$sets_table} ADD COLUMN reviewer_comment text DEFAULT NULL" );
        $this->ensure_column( $sets_table, 'revision_history', "ALTER TABLE {$sets_table} ADD COLUMN revision_history longtext DEFAULT NULL" );
        ...
        // Default 'pending', NOT 'approved'. A question that nobody has looked at is
        // not approved, and defaulting it that way meant every row inserted without
        // an explicit status arrived pre-approved — which is why the approval screen
        // showed a queue of "approved" work nobody had reviewed.
        // These belong to the question-bank rebuild and used to exist ONLY as a
        // migration. A migration runs once and records that it has run — so when
        // this table was dropped and recreated from the base schema, the migration
        // saw itself as already applied and never re-added them. Every query that
        // joins a question to its set then failed with "Unknown column
        // question_set_id", taking the approvals screen down with it.
        //
        // Declared here instead, where the check is against the table itself
        // rather than against a version number, so the columns are correct
        // whatever state the table is in.
        $this->ensure_column( $questions_table, 'question_set_id', "ALTER TABLE {$questions_table} ADD COLUMN question_set_id bigint(20) unsigned DEFAULT NULL" );
        $this->ensure_column( $questions_table, 'sequence', "ALTER TABLE {$questions_table} ADD COLUMN sequence int(11) NOT NULL DEFAULT 0" );
        $this->ensure_column( $questions_table, 'source_method', "ALTER TABLE {$questions_table} ADD COLUMN source_method varchar(20) NOT NULL DEFAULT 'manual'" );
        $this->ensure_column( $questions_table, 'marking_guide', "ALTER TABLE {$questions_table} ADD COLUMN marking_guide longtext DEFAULT NULL" );
        $this->ensure_column( $questions_table, 'reviewer_comment', "ALTER TABLE {$questions_table} ADD COLUMN reviewer_comment text DEFAULT NULL" );
        $this->ensure_column( $questions_table, 'parent_id', "ALTER TABLE {$questions_table} ADD COLUMN parent_id bigint(20) unsigned DEFAULT NULL" );

        $this->ensure_column( $questions_table, 'approval_status', "ALTER TABLE {$questions_table} ADD COLUMN approval_status varchar(20) NOT NULL DEFAULT 'pending'" );
        ...
        // Theory sub-questions: a parent theory question (parent_id = 0 or NULL)
        // groups its children (1A, 1B, 1C) which point back to it via parent_id.
        $this->ensure_column( $questions_table, 'parent_id', "ALTER TABLE {$questions_table} ADD COLUMN parent_id bigint(20) unsigned DEFAULT NULL AFTER question_type" );
        $this->ensure_column( $questions_table, 'part_label', "ALTER TABLE {$questions_table} ADD COLUMN part_label varchar(10) DEFAULT '' AFTER parent_id" );
        $this->ensure_index( $questions_table, 'parent', "ALTER TABLE {$questions_table} ADD KEY parent (parent_id)" );

        // Exam papers: store the intended objective question count separately
        // from question_count (which includes theory_min). This lets the delivery
        // engine know exactly how many objective questions each student should
        // see, regardless of how many theory questions are in the pool.
        $papers_t = $wpdb->prefix . 'educbt_exam_papers';
        $this->ensure_column( $papers_t, 'objective_per_student', "ALTER TABLE {$papers_t} ADD COLUMN objective_per_student int(11) NOT NULL DEFAULT 0" );
        $this->ensure_column( $papers_t, 'closes_at', "ALTER TABLE {$papers_t} ADD COLUMN closes_at datetime DEFAULT NULL AFTER scheduled_at" );

        // One-time migration: create child records in educbt_questions for
        // theory sub-questions that exist in question_sub_items but don't
        // have corresponding child rows yet. This lets the delivery, marking,
        // and answer-saving systems use real question IDs for sub-questions.
        $sub_items_t  = $wpdb->prefix . 'educbt_question_sub_items';
        $questions_t  = $wpdb->prefix . 'educbt_questions';
        $has_children = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$questions_t} WHERE parent_id IS NOT NULL AND parent_id > 0" );
        $has_subitems = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$sub_items_t}" );

        if ( $has_subitems > 0 && $has_children === 0 ) {
            $subs = (array) $wpdb->get_results(
                "SELECT si.*, q.question_set_id, q.subject_id, q.subject, q.class,
                        q.status, q.approval_status, q.created_by_staff
                 FROM {$sub_items_t} si
                 INNER JOIN {$questions_t} q ON q.id = si.question_id
                 ORDER BY si.question_id, si.sequence ASC",
                ARRAY_A
            );
            foreach ( $subs as $sub ) {
                $wpdb->insert(
                    $questions_t,
                    [
                        'question_set_id'  => absint( $sub['question_set_id'] ),
                        'school_id'        => absint( $sub['school_id'] ),
                        'subject_id'       => $sub['subject_id'] ?? null,
                        'subject'          => $sub['subject'] ?? '',
                        'class'            => $sub['class'] ?? '',
                        'question_text'    => (string) $sub['text'],
                        'question_type'    => 'theory',
                        'parent_id'        => absint( $sub['question_id'] ),
                        'part_label'       => (string) $sub['label'],
                        'marks'            => (float) $sub['marks'],
                        'status'           => (string) ( $sub['status'] ?? 'active' ),
                        'approval_status'  => (string) ( $sub['approval_status'] ?? 'pending' ),
                        'created_by_staff' => absint( $sub['created_by_staff'] ?? 0 ),
                        'sequence'         => absint( $sub['sequence'] ),
                    ]
                );
            }
        }
