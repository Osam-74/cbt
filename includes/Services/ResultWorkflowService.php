...
class ResultWorkflowService {
     ...
     */
    public function publish_class( int $school_id, int $class_id, int $session_id, int $term_id, int $actor_id, bool $force = false ): array {
        $checks = $this->pre_publish_checks( $school_id, $class_id, $session_id, $term_id );

        // The principal may choose to publish despite gaps — a student who was
        // absent is scored zero, not held hostage to a missing entry. The checks
        // are surfaced as warnings, not as a hard block, when force is set.
        if ( ! empty( $checks ) && ! $force ) {
            return [ 'success' => false, 'errors' => $checks, 'warnings' => $checks ];
        }

        // Even with force, the results must be in the APPROVED state to publish.
        // You cannot force-publish unapproved results — that would skip the
        // entire review chain.
        $must_approve = false;
        foreach ( $checks as $check ) {
            if ( str_starts_with( $check, 'not_approved:' ) ) {
                $must_approve = true;
                break;
            }
        }
        if ( $must_approve ) {
            return [ 'success' => false, 'errors' => [ 'not_approved' ], 'warnings' => $checks ];
        }
        ...
        // Apply auto-remarks to fill in any empty remark fields based on score ranges.
        $remark_service = new RemarkService();
        $remark_service->apply_auto_remarks( $school_id, $class_id, $term_id );
        ...
        return [ 'success' => true, 'published' => $result['moved'], 'warnings' => $force ? $checks : [] ];
    }
