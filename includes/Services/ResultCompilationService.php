...
class ResultCompilationService {
    ...
    public function compile_class( int $school_id, int $class_id, int $session_id, int $term_id ): array {
        ...
        // Compiling nothing is not success. It used to report as success with a
        // count of zero, so a principal whose teachers had not entered any scores
        // was told the class had been compiled.
        if ( $compiled === 0 ) {
            return [
                'success'  => false,
                'error'    => empty( $subject_ids ) ? 'no_scores_recorded' : 'nothing_compiled',
                'subjects' => 0,
                'students' => 0,
                'skipped'  => $skipped,
            ];
    ...
    public function report_card( int $school_id, int $student_id, int $term_id ): array {
        ...
        $subjects        = Schema::table( 'subjects_v2' );
        $student_subs    = Schema::table( 'student_subjects' );
        $classes         = Schema::table( 'classes' );
        $enrollments     = Schema::table( 'enrollments' );

        $summary = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT tr.*, c.display_name AS class_name
        ...
        // If class_name wasn't in term_results, try enrollment
        if ( empty( $summary['class_name'] ) ) {
            $class_name = (string) $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT c.display_name FROM {$enrollments} e
                     INNER JOIN {$classes} c ON c.id = e.class_id
                     WHERE e.student_id = %d AND e.school_id = %d AND e.status = 'active'
        ...
        // Get the session_id from the term to match student_subjects
        $terms_table = Schema::table( 'terms' );
        $session_id = (int) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT session_id FROM {$terms_table} WHERE id = %d AND school_id = %d",
                $term_id,
                $school_id
            )
        );

        // LEFT JOIN student_subjects so ALL registered subjects appear,
        // even those without compiled scores (shown as dash on the sheet).
        $lines = (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT sr.*, s.name AS subject_name, s.code AS subject_code,
                        CASE WHEN sr.id IS NULL THEN 0 ELSE 1 END AS has_scores
                 FROM {$student_subs} ss
                 INNER JOIN {$subjects} s ON s.id = ss.subject_id
                 LEFT JOIN {$subject_results} sr ON sr.subject_id = ss.subject_id
                     AND sr.student_id = ss.student_id AND sr.term_id = %d AND sr.school_id = %d
                 WHERE ss.student_id = %d AND ss.school_id = %d
                 ORDER BY s.name ASC",
                $term_id,
                $school_id,
                $student_id,
