...
class AssessmentService {
    ...
    public function entry_sheet( int $school_id, int $component_id, array $context ): array {
        ...
        // A student is eligible for score entry if they are EITHER registered
        // for this subject in student_subjects OR simply enrolled in the class.
        // The can_open() gate that lets students start exams already uses this
        // same OR logic, so the score sheet must match — otherwise a student
        // who sat the exam (via class enrolment) is invisible to the teacher
        // when entering CA scores.
        //
        // We try registered students first. If none are registered for this
        // subject in this class, we fall back to all active enrolments — the
        // same COALESCE pattern used on the teacher dashboard.
        $registered_count = (int) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(DISTINCT rs.student_id)
                 FROM {$registered} rs
                 INNER JOIN {$enrollments} e ON e.student_id = rs.student_id
                  AND e.class_id = %d AND e.session_id = %d AND e.status = 'active'
                 WHERE rs.subject_id = %d",
                absint( $context['class_id'] ?? 0 ),
        ...
        if ( $registered_count > 0 ) {
            // Subject registration exists — show only registered students.
        ...
        // No subject registration for this class — fall back to all enrolled students.
        return (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT st.id AS student_id, st.admission_number, st.first_name, st.last_name,
                        sc.score, sc.source, sc.max_score
    ...
    /**
     * The full broadsheet: every student x every component on one screen.
     *
     * Returns a flat list of students, each carrying an associative array of
     * component_id => { score, source, max_score }.  CBT-sourced exam marks
     * arrive read-only; manual CA marks are editable.
     *
     * @return array<int,array<string,mixed>>
     */
    public function full_entry_sheet( int $school_id, array $context ): array {
        global $wpdb;
        ...
        $registered    = Schema::table( 'student_subjects' );
        $scores        = Schema::table( 'assessment_scores' );
        $components_t  = Schema::table( 'assessment_components' );
        ...
        // All active components for this school — CA and exam alike.
        ...
        // Only show the exam component column when an exam paper has actually
        // been approved (published) for this subject + class.  If no paper has
        // reached 'published' status or beyond, the exam column is hidden —
        // there is nothing to show and an empty box would only confuse.
        $has_exam = false;
        ...
        if ( $has_exam ) {
            $papers_t = Schema::table( 'exam_papers' );
            $approved_paper = (int) $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(*) FROM {$papers_t}
            ...
            // No approved exam paper yet — drop the exam component from the grid.
            if ( $approved_paper === 0 ) {
        ...
        // Resolve the student list: registered for this subject OR enrolled
        // (same COALESCE pattern as entry_sheet above).
        $registered_count = (int) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(DISTINCT rs.student_id)
                 FROM {$registered} rs
                 INNER JOIN {$enrollments} e ON e.student_id = rs.student_id
                  AND e.class_id = %d AND e.session_id = %d AND e.status = 'active'
                 WHERE rs.subject_id = %d",
                $class_id, $session_id, $subject_id
            )
        );
        ...
        // Fetch all existing scores for these students in this subject/term.
        $student_ids = array_map( 'absint', array_column( $students, 'student_id' ) );
        $placeholders = implode( ',', array_fill( 0, count( $student_ids ), '%d' ) );

        $existing = (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT student_id, component_id, score, source, max_score
                 FROM {$scores}
                 WHERE school_id = %d AND subject_id = %d AND term_id = %d
                   AND student_id IN ({$placeholders})",
                array_merge( [ $school_id, $subject_id, $term_id ], $student_ids )
            ),
            ARRAY_A
        );

        // Index: [student_id][component_id] => { score, source }
        $score_map = [];
        foreach ( $existing as $sc ) {
            $sid = absint( $sc['student_id'] );
            $cid = absint( $sc['component_id'] );
            $score_map[ $sid ][ $cid ] = [
                'score'     => $sc['score'] !== null ? (float) $sc['score'] : null,
                'source'    => (string) $sc['source'],
                'max_score' => (float) $sc['max_score'],
            ];
        }

        // Attach component scores to each student.
        foreach ( $students as &$student ) {
            $sid = absint( $student['student_id'] );
            $student['components'] = [];

            foreach ( $components as $comp ) {
                $cid = absint( $comp['id'] );
                $entry = $score_map[ $sid ][ $cid ] ?? null;

                $student['components'][ $cid ] = [
                    'score'     => $entry['score'] ?? null,
                    'source'    => $entry['source'] ?? '',
                    'max_score' => (float) $comp['max_score'],
                    'is_exam'   => ! empty( $comp['is_exam'] ),
