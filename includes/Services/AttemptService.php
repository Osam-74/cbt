...
class AttemptService {
    ...
    private function build_question_order( int $school_id, int $paper_id, array $paper ): array {
        ...
        $rows = (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT q.id, q.passage_id, q.passage_position, q.question_type, q.parent_id, q.section
        ...
        // Theory questions must never be shuffled — they display in the
        // order the teacher submitted them. Only objective questions are
        // randomized (when the paper has shuffle_questions enabled).
        $objective_rows = [];
        $theory_rows    = [];

        // Cloze items are objective but must NOT be shuffled. Their numbering runs
        // with the passage — gap 3 is the third blank in the text — so reordering
        // them makes the passage unreadable and the paper unanswerable. They are
        // grouped with theory here purely because that is the branch that preserves
        // submitted order.
        foreach ( $rows as $row ) {
            $is_cloze = strpos( (string) ( $row['section'] ?? '' ), ':cloze' ) !== false;

            if ( (string) ( $row['question_type'] ?? '' ) === 'theory' || $is_cloze ) {
                $theory_rows[] = $row;
            } else {
                $objective_rows[] = $row;
            }
        ...
        // A theory question whose parent is ALSO in the pool is a sub-question
        // (1A, 1B, 1C…) — it is displayed under its parent at delivery time, so
        // it must NOT also appear as a standalone question. Filter it out to
        // avoid showing the same question twice.
        $theory_ids_in_pool = array_map( function ( $r ) { return absint( $r['id'] ); }, $theory_rows );
        $standalone_theory  = [];

        foreach ( $theory_rows as $row ) {
            $parent_id = absint( $row['parent_id'] ?? 0 );
            if ( $parent_id > 0 && in_array( $parent_id, $theory_ids_in_pool, true ) ) {
                // This is a sub-question whose parent is also in the pool —
                // it will be shown as a child of the parent. Skip it here.
                continue;
            }
            $standalone_theory[] = $row;
        }

        // The pool may contain MORE objective questions than each student
        // should see. The paper's question_count is the per-student total;
        // subtract the standalone theory count to get how many objective
        // questions to draw. Each student gets a different random subset.
        // Use objective_per_student if it was stored (timetable-built papers).
        // Fall back to question_count minus theory for legacy papers.
        $objective_per_student = absint( $paper['objective_per_student'] ?? 0 );
        if ( $objective_per_student > 0 ) {
            $per_student_objective = $objective_per_student;
        } else {
            $per_student_objective = absint( $paper['question_count'] ) - count( $standalone_theory );
        }
        if ( $per_student_objective < 0 ) {
            $per_student_objective = 0;
        }

        if ( count( $objective_rows ) > $per_student_objective && $per_student_objective > 0 ) {
            shuffle( $objective_rows );
            $objective_rows = array_slice( $objective_rows, 0, $per_student_objective );
        }

        $ordered = [];

        // Objective questions go through the passage-aware shuffle.
        if ( ! empty( $objective_rows ) ) {
            $ordered = ( new PassageService() )->order_for_delivery( $objective_rows, ! empty( $paper['shuffle_questions'] ) );
        }

        // Theory questions appended in their original sort_order (no shuffle).
        // Sub-questions (children) are loaded at delivery time and displayed
        // under their parent — they are not in this order as standalone items.
        foreach ( $standalone_theory as $row ) {
            $ordered[] = absint( $row['id'] );
        }
    ...
    public function deliver( int $school_id, int $attempt_id ): array {
        ...
        $rows = (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id, question_text, image_reference, question_type, marks, passage_id, parent_id, part_label, section
        ...
        // Fetch theory sub-questions (children) whose parent_id is in the order.
        // These are grouped under their parent for display.
        $child_rows = (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id, question_text, image_reference, question_type, marks, passage_id, parent_id, part_label, section
                 FROM {$questions} WHERE parent_id IN ({$placeholders}) ORDER BY parent_id, id ASC",
                $order
            ),
            ARRAY_A
        );
        ...
        foreach ( $child_rows as $row ) {
            $children_by_parent[ absint( $row['parent_id'] ) ][] = $row;
        }
        ...
        foreach ( $order as $question_id ) {
            ...
            $q_type   = (string) $question['question_type'];
            ...
            $item = [
                ...
                // The exam room needs to know which WAEC section an item belongs to:
                // essay alternatives are presented as a choice rather than as a run
                // of questions, and cloze gaps must not be reordered.
                'section'    => (string) ( $question['section'] ?? '' ),
                'options'    => $choices,
            ];

            // For theory questions, attach any sub-questions (1A, 1B, 1C…).
            // These render together on one screen as a group.
            if ( $q_type === 'theory' && ! empty( $children_by_parent[ $question_id ] ) ) {
                $item['children'] = array_map( function ( $child ) {
                    return [
                        'id'    => absint( $child['id'] ),
                        'text'  => (string) $child['question_text'],
                        'image' => (string) $child['image_reference'],
                        'marks' => (float) $child['marks'],
                        'label' => (string) $child['part_label'],
                    ];
                }, $children_by_parent[ $question_id ] );
            }
    ...
    public function submit( int $school_id, int $attempt_id, string $reason = self::REASON_MANUAL, string $session_token = '' ): array {
        ...
        $closed = $this->close( $school_id, $attempt_id, $reason );

        if ( ! $closed ) {
            // The UPDATE matched zero rows — the status was not 'in_progress'
            // when we tried to close it. This can happen under a race (two tabs
            // submitting at once) or if the sweeper already closed it between
            // our get() and our close(). Either way, re-read the attempt so we
            // report the true status rather than claiming success.
            $fresh = $this->get( $school_id, $attempt_id );
            return [
                'success' => true,
                'status'  => (string) ( $fresh['status'] ?? self::STATUS_SUBMITTED ),
            ];
        }
    ...
    public function log_event( int $school_id, int $attempt_id, string $type, array $payload = [] ): void {
        ...
        // Each of these is something an invigilator would want to know about and
        // can act on. 'right_click' and 'tab_hidden' were being warned about on the
        // student's screen but never recorded, so the warning said "this has been
        // logged" when nothing had been.
        $allowed = [ 'window_blur', 'second_session', 'resumed', 'right_click', 'tab_hidden', 'fullscreen_exit' ];
        ...
        $wpdb->query(
            $wpdb->prepare(
                'UPDATE ' . Schema::table( 'attempts' ) . ' SET integrity_count = integrity_count + 1 WHERE id = %d',
                $attempt_id
            )
        );
    }
    ...
    /**
     * Delete a student's attempt so they can start fresh.
     *
     * Called by a teacher or admin who has verified that the student's session
     * ended unfairly (e.g., the timing bug that ended CA tests immediately).
     * Removes the attempt row and all its answers, allowing the student to
     * start again as if they never had.
     */
    public function reset_attempt( int $school_id, int $attempt_id ): array {
        global $wpdb;

        $attempt = $this->get( $school_id, $attempt_id );

        if ( ! $attempt ) {
            return [ 'success' => false, 'errors' => [ 'attempt_not_found' ] ];
        }

        // Delete the attempt's answers first (FK safety)
        $wpdb->delete(
            Schema::table( 'attempt_answers' ),
            [ 'attempt_id' => $attempt_id ],
            [ '%d' ]
        );
    ...
    /**
     * Check whether a question belongs to this attempt — either directly
     * (it is in question_order) or as a child of a theory question that is.
     *
     * Theory sub-questions (1A, 1B, 1C) are delivered as children of their
     * parent and are NOT listed separately in question_order. Without this
     * check, saving a sub-question answer would be rejected as "not on this
     * paper" — silently dropping every theory sub-question response.
     */
    public function question_or_child_on_attempt( array $attempt, int $question_id ): bool {
        if ( $this->question_on_attempt( $attempt, $question_id ) ) {
            return true;
        }
        ...
        $parent_id = $wpdb->get_var(
            $wpdb->prepare( "SELECT parent_id FROM {$questions} WHERE id = %d", $question_id )
        );

        if ( $parent_id && absint( $parent_id ) > 0 ) {
            return $this->question_on_attempt( $attempt, absint( $parent_id ) );
        }
    ...
    /**
     * Paper data for the ExamController. Wraps the private paper() method
     * so the controller can access it without duplicating the query.
     */
    public function paper_for_controller( int $school_id, int $paper_id ): array {
        return $this->paper( $school_id, $paper_id );
    }
