...
class PromotionService {
    ...
    /**
     * Promote or demote a single student — search by name or admission number,
     * pick a target class, and move the enrollment. No batch needed.
     *
     * The principal does not need to run a full evaluation to move one student
     * who joined mid-year or was misplaced. This bypasses the rule engine
     * entirely — it's a direct, human-driven reassignment.
     *
     * @return array{success:bool,error?:string}
     */
    public function move_student( int $school_id, int $student_id, int $to_class_id, string $outcome, string $reason, int $actor_id ): array {
        global $wpdb;
        ...
        $student = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT id, admission_number, CONCAT(first_name, ' ', last_name) AS name FROM {$students_t} WHERE id = %d AND school_id = %d",
                $student_id,
                $school_id
            ),
            ARRAY_A
        ...
        // Verify the target class exists and belongs to this school.
        $target = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT id, display_name, level_id FROM {$classes} WHERE id = %d AND school_id = %d AND status = 'active'",
                $to_class_id,
                $school_id
            ),
            ARRAY_A
        ...
        if ( ! $target ) {
            return [ 'success' => false, 'error' => 'target_class_not_found' ];
        }

        // Find the student's current active enrollment.
        $current = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT id, class_id, session_id FROM {$enrollments} WHERE school_id = %d AND student_id = %d AND status = 'active' ORDER BY id DESC LIMIT 1",
                $school_id,
                $student_id
            ),
            ARRAY_A
        ...
        $session_id = $current ? absint( $current['session_id'] ) : 0;

        // Get the current session if no enrollment exists.
        if ( $session_id === 0 ) {
            $year  = new AcademicYearService();
            $sess  = $year->current_session( $school_id );
            $session_id = absint( $sess['id'] ?? 0 );
        }
        ...
        try {
            ...
            if ( $current ) {
                $wpdb->update(
                    $enrollments,
                    [ 'status' => 'transferred', 'ended_at' => current_time( 'mysql', true ) ],
                    [ 'id' => absint( $current['id'] ) ],
                    [ '%s', '%s' ],
                    [ '%d' ]
                );
            }
    ...
    /**
     * Search students by name or admission number for individual promotion.
     *
     * @return array<int,array<string,mixed>>
     */
    public function search_students( int $school_id, string $query ): array {
        global $wpdb;
        ...
        return (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT s.id, s.admission_number, CONCAT(s.first_name, ' ', s.last_name) AS name,
                        s.status, e.class_id, c.display_name AS class_name
                 FROM {$students_t} s
                 LEFT JOIN {$enrollments} e ON e.student_id = s.id AND e.status = 'active'
                 LEFT JOIN {$classes} c ON c.id = e.class_id
                 WHERE s.school_id = %d AND (
                       s.admission_number LIKE %s
                 ...
                 ORDER BY s.last_name ASC LIMIT 25",
