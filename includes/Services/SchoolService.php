...
class SchoolService {
    ...
    public function create_school( array $data ): int {
        ...
        // Seed the standard NERDC offering into subjects_v2 — the table the portal
        // actually reads. The legacy seeding above writes to the older subjects
        // table, so without this a newly onboarded school opened its Subjects page
        // to an empty list.
        //
        // TO STOP SUBJECT SEEDING ENTIRELY, delete the two lines below.
        // It is safe to leave them: the seeder only ever runs for a school that has
        // no subjects at all, and records a flag once it has run.
        ( new SubjectSeederService() )->maybe_seed( $school_id );
    ...
    /**
     * Update a school's own details.
     *
     * The admin edit form has always called this; the method was never written, so
     * saving a school from WP Admin raised a fatal error and the edit was lost.
     *
     * Only columns that exist on the table are written, and only keys the caller
     * actually supplied — so a form that posts a subset cannot blank the fields it
     * did not include.
     */
    public function update_school( int $school_id, array $data ): bool {
        ...
        // Settings columns hold JSON, so they are encoded rather than passed through.
        foreach ( [ 'academic_settings', 'report_settings' ] as $json_column ) {
            if ( array_key_exists( $json_column, $data ) ) {
                $fields[ $json_column ] = is_string( $data[ $json_column ] )
                    ? $data[ $json_column ]
                    : (string) wp_json_encode( $data[ $json_column ] );
                $formats[]              = '%s';
            }
        }
        ...
        $updated = $wpdb->update( $table, $fields, [ 'id' => $school_id ], $formats, [ '%d' ] );
