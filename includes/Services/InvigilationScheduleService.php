...
class InvigilationScheduleService {
    ...
    public function papers( int $school_id, int $series_id ): array {
        ...
        return (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT p.id, p.scheduled_at, p.duration_seconds, p.status, p.access_code, p.is_practice, p.delivery_mode,
        ...
        return (array) $wpdb->get_results(
            $wpdb->prepare(
                "SELECT s.id, CONCAT(s.first_name, ' ', s.last_name) AS name, s.role_slug,
                 ...
                 FROM {$staff} s
                 WHERE s.school_id = %d AND s.status = 'active' AND s.role_slug = 'educbt_teacher'
    ...
    public function propose( int $school_id, int $series_id ): array {
        ...
        foreach ( $papers as $paper ) {
            ...
            // Delete any stale row for this paper first, then insert fresh.
            $wpdb->delete( $invig, [ 'paper_id' => absint( $paper['id'] ), 'school_id' => $school_id ], [ '%d', '%d' ] );

            $wpdb->insert(
                $invig,
                [
                    'school_id'     => $school_id,
                    'paper_id'      => absint( $paper['id'] ),
                    'staff_id'      => $chosen,
                    'assigned_mode' => 'auto',
                ],
                [ '%d', '%d', '%d', '%s' ]
     ...
     */
    public function reassign( int $school_id, int $paper_id, int $staff_id, bool $force = false ): array {
        global $wpdb;
        ...
        // Delete any existing invigilator row for this paper first, then insert.
        // The unique key is (paper_id), so ON DUPLICATE KEY would work, but
        // cleaning up explicitly avoids stale rows from before the migration.
        $wpdb->delete( $invig, [ 'paper_id' => $paper_id, 'school_id' => $school_id ], [ '%d', '%d' ] );
