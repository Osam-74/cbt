...
class AcademicStructureService {
    ...
    public function create_class( int $school_id, int $level_id, string $arm, ?int $department_id = null, int $capacity = 0 ): array {
        ...
        // Look at EVERY row for this key, not just active ones. A class that was
        // removed earlier is only soft-deleted, so the row survives and the UNIQUE
        // key still rejects the insert — but the active-only check above saw
        // nothing, so the failure came back as "that class already exists" for a
        // class the user could not see anywhere. Revive it instead.
        $existing = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT id, status FROM {$classes_table}
                 WHERE school_id = %d AND level_id = %d AND arm = %s
                   AND COALESCE(department_id, 0) = %d
                 LIMIT 1",
                $school_id,
                $level_id,
                $arm,
        ...
        if ( $existing ) {
            ...
            return [ 'success' => true, 'class_id' => absint( $existing['id'] ), 'restored' => true ];
        }
        ...
        if ( ! $inserted ) {
            if ( function_exists( 'error_log' ) && $wpdb->last_error !== '' ) {
                error_log( 'EduCBT: class insert failed — ' . $wpdb->last_error );
            }

            return [ 'success' => false, 'error' => 'class_could_not_be_saved' ];
        }
    ...
    public function validate_registration( int $school_id, int $level_id, ?int $department_id, array $subject_ids ): array {
        ...
        // A subject is compulsory because the school flagged it "every student
        // must offer this" (is_compulsory) — schools set their own subject codes
        // freely when creating subjects, so there is no guarantee any subject in
        // a school's data actually carries the demo seed's codes (ENG-J, MTH-J,
        // ENG, MTH, CVE). Checking those literal codes against a school's real
        // codes would find no match and block EVERY registration attempt, no
        // matter what the student picks. Instead, check by subject ID against
        // whichever subjects for this level/department are actually compulsory
        // — the same rule offering_split() uses to decide what is "core" in the
        // first place, so the two stay consistent.
        $offerings_for_level = $this->offerings( $school_id, $level_id, $department_id );

        foreach ( $offerings_for_level as $offer ) {
            $is_mandatory = ! empty( $offer['is_compulsory'] )
                || in_array( (string) $offer['code'], $rules['mandatory_codes'], true );

            if ( $is_mandatory && ! in_array( absint( $offer['id'] ), $subject_ids, true ) ) {
                $errors[] = 'missing_compulsory:' . (string) $offer['name'];
            }
        }
    ...
    public function set_student_electives( int $school_id, int $student_id, int $session_id, array $elective_ids ): array {
        ...
        if ( $this->registration_locked( $school_id, $session_id ) ) {
            error_log( 'EduCBT set_student_electives: registration_locked school=' . $school_id . ' session=' . $session_id );
        ...
        if ( isset( $view['error'] ) ) {
            error_log( 'EduCBT set_student_electives: view error=' . $view['error'] . ' student=' . $student_id . ' session=' . $session_id );
        ...
        if ( ! empty( $unknown ) ) {
            error_log( 'EduCBT set_student_electives: unknown subjects=' . implode( ',', $unknown ) . ' student=' . $student_id );
        ...
        $enrollment = $wpdb->get_row(
            $wpdb->prepare(
                'SELECT e.class_id, c.level_id, c.department_id FROM ' . Schema::table( 'enrollments' ) . ' e
                 INNER JOIN ' . Schema::table( 'classes' ) . ' c ON c.id = e.class_id
                 WHERE e.student_id = %d AND e.session_id = %d AND e.status = %s LIMIT 1',
                $student_id,
                $session_id,
                'active'
            ),
        ...
        if ( ! $enrollment ) {
            error_log( 'EduCBT set_student_electives: no active enrollment student=' . $student_id . ' session=' . $session_id );
            return [ 'success' => false, 'errors' => [ 'not_enrolled' ] ];
        }
        ...
        if ( ! $check['valid'] ) {
            error_log( 'EduCBT set_student_electives: validation failed errors=[' . implode( ', ', $check['errors'] ) . '] student=' . $student_id . ' final=[' . implode( ',', $final ) . ']' );
            return [ 'success' => false, 'errors' => $check['errors'] ];
        }
        ...
        if ( ! empty( $dropped ) ) {
            error_log( 'EduCBT set_student_electives: cannot drop assessed subjects=[' . implode( ',', $dropped ) . '] student=' . $student_id );
        ...
        foreach ( $final as $subject_id ) {
            ...
            if ( ! $inserted && $wpdb->last_error !== '' ) {
                $insert_errors[] = 'subject ' . $subject_id . ': ' . $wpdb->last_error;
            }
        }
        ...
        if ( ! empty( $insert_errors ) ) {
            error_log( 'EduCBT set_student_electives: DB insert errors: ' . implode( '; ', $insert_errors ) );
        }
