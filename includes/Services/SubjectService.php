...
class SubjectService {
    ...
    public function update_subject( int $school_id, int $subject_id, array $data ): bool {
        if ( isset( $data['subject_name'] ) ) {
            $data['subject_name'] = $this->canonicalize_subject_name( $school_id, (string) $data['subject_name'] );
            if ( $data['subject_name'] === '' ) {
                return false;
            }
        }
        return $this->repository->update_subject( $school_id, $subject_id, $data );
    }

    /**
     * Delete a subject. Returns array with success status and reference count.
     *
     * @return array{success:bool,references:int}
     */
    public function delete_subject( int $school_id, int $subject_id ): array {
        $subjects = $this->list_subjects( $school_id );
        $subject = null;
        ...
        if ( ! $subject ) {
            return [ 'success' => false, 'references' => 0 ];
        }

        $refs = $this->repository->count_question_references( $school_id, $subject['subject_name'] );
        $deleted = $this->repository->delete_subject( $school_id, $subject_id );

        return [ 'success' => $deleted, 'references' => $refs ];
    }

    /**
     * Merge a duplicate subject into a target. All questions referencing
     * the source subject name are updated to the target name, then the
     * source is deleted.
     *
     * @return bool
     */
    public function merge_subjects( int $school_id, int $source_id, int $target_id ): bool {
        ...
        return $this->repository->merge_subjects( $school_id, $source_id, $target_id );
    }
