...
class EmailQueueService {
    ...
    public function queue( int $school_id, string $recipient, string $subject, string $body, ?string $send_after = null ): int {
        ...
        // Placeholder addresses generated for students/guardians without a real email.
        if ( str_ends_with( strtolower( $recipient ), '@local.educbt' ) ) {
            return 0;
        }
    ...
    /**
     * Queue an email and immediately attempt to send it.
     * Falls back to the cron-based drain if wp_mail is not available yet.
     */
    public function queue_and_send( int $school_id, string $recipient, string $subject, string $body, ?string $send_after = null ): int {
        $id = $this->queue( $school_id, $recipient, $subject, $body, $send_after );
        if ( $id > 0 ) {
            $this->drain();
        }
        return $id;
