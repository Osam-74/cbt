...
class AcademicYearService {
    ...
    /**
     * Resolve the term to work in, without depending on `is_current` being set.
     *
     * `current_term()` answers only one question: which row carries the flag. That
     * flag is set at onboarding and when an admin switches session, but a session
     * created directly from the sessions screen, or a term row rebuilt afterwards,
     * leaves every term at is_current = 0. Anything that hard-failed on a null term
     * then reported itself as a generic save failure, which is what made the
     * question bank look broken while the rest of the portal worked.
     *
     * It can also point at a term belonging to a DIFFERENT session, which quietly
     * files new work under the wrong (session, term) pair.
     *
     * Resolution order, all scoped to the session when one is given:
     *   1. the flagged term, if it belongs to that session
     *   2. the term whose date range contains today
     *   3. the lowest term_order term of the session
     *
     * @return array<string,mixed>|null
     */
    public function resolve_current_term( int $school_id, int $session_id = 0 ): ?array {
        global $wpdb;

        $table = Schema::table( 'terms' );

        $flagged = $this->current_term( $school_id );

        if ( $flagged && ( $session_id <= 0 || absint( $flagged['session_id'] ?? 0 ) === $session_id ) ) {
            return $flagged;
        }
            ...
            $wpdb->prepare(
                "SELECT * FROM {$table}
                 WHERE school_id = %d AND session_id = %d
                   ...
                   AND %s BETWEEN starts_on AND ends_on
                 ORDER BY term_order ASC LIMIT 1",
                $school_id,
                $session_id,
                $today
            ),
