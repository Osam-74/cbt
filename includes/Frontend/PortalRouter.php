...
class PortalRouter {
    ...
    public static function sections(): array {
        return [
            'school' => [
                ...
                'student'     => [ 'capability' => Capabilities::VIEW_STUDENTS, 'label' => '' ],
                ...
                'signatures'  => [ 'capability' => Capabilities::MANAGE_SCHOOL, 'label' => 'Signature' ],
                'remarks'     => [ 'capability' => Capabilities::MANAGE_SCHOOL, 'label' => 'Remarks' ],
                'transcript-print' => [ 'capability' => Capabilities::ISSUE_TRANSCRIPT, 'label' => '' ],
                'review'      => [ 'capability' => Capabilities::APPROVE_RESULTS, 'label' => '' ],
                'notices'     => [ 'capability' => Capabilities::SEND_ANNOUNCEMENT, 'label' => '' ],
                'activity'    => [ 'capability' => Capabilities::VIEW_ACTIVITY_LOG, 'label' => 'Activity Log' ],
                'settings'    => [ 'capability' => Capabilities::MANAGE_SCHOOL, 'label' => 'School Settings' ],
            ],
            'exams' => [
                ...
                'papers'       => [ 'capability' => Capabilities::MANAGE_PAPERS, 'label' => 'Exam Papers' ],
                ...
                'sessions'     => [ 'capability' => Capabilities::VIEW_EXAMS, 'label' => 'Test/Exam Sessions', 'requires_school_wide' => true ],
                'marking'      => [ 'capability' => Capabilities::VIEW_EXAMS, 'label' => 'Marking Status' ],
                // Hidden from nav — merged into Test/Exam Sessions.
                'invigilate'   => [ 'capability' => Capabilities::INVIGILATE, 'label' => '' ],
                // Hidden from nav — moved to School area.
                'broadsheet'   => [ 'capability' => Capabilities::VIEW_BROADSHEET, 'label' => '' ],
            ],
            'teacher' => [
                ''             => [ 'capability' => Capabilities::ENTER_SCORES, 'label' => 'Dashboard' ],
                'registration' => [ 'capability' => Capabilities::ENTER_SCORES, 'label' => 'Subject Registration' ],
                'classes'      => [ 'capability' => Capabilities::ENTER_SCORES, 'label' => 'My Assignments' ],
                ...
                // Hidden from the menu, but the ROUTE stays: an empty label is how
                // this router keeps a page reachable without listing it.
                //
                // Setting questions for a CA test and setting them for the terminal
                // examination are the same job, done in the same place. Two menus for
                // one job meant a teacher notified about a CA window went looking
                // under Class Tests, found a different form with its own rules, and
                // set questions that never reached the window the office had opened.
                //
                // Everything now happens in the Question Bank, scoped to whichever
                // window is open. Old links and bookmarks still resolve.
                'tests'        => [ 'capability' => Capabilities::ENTER_SCORES, 'label' => '' ],
                ...
                // Hidden from nav — reached via links from Subject Results.
                'responses'    => [ 'capability' => Capabilities::ENTER_SCORES, 'label' => '' ],
                ...
                'signatures'   => [ 'capability' => Capabilities::ENTER_SCORES, 'label' => 'Signature' ],
                // A class teacher sets up auto-remark ranges and can override an
                // individual student's remark. Scoped to their own class(es) — Gate
                // checks REMARK_RESULTS against the specific class/student.
                'remarks'      => [ 'capability' => Capabilities::REMARK_RESULTS, 'label' => 'Remarks', 'requires_class_teacher' => true, 'exclude_exam_officer' => true ],
                // Hidden from nav — reached via the "View / Print" link on Class
                // Results. Lets a class teacher open a student's report sheet as
                // soon as it is approved, without waiting for publication.
                'report'       => [ 'capability' => Capabilities::REMARK_RESULTS, 'label' => '' ],
                // Hidden from nav — reached via the "Download All Reports" button.
                // Renders ALL students' report sheets on one printable page.
                'bulk-report'  => [ 'capability' => Capabilities::REMARK_RESULTS, 'label' => '' ],
            ],
            'student' => [
                ...
                'subjects'  => [ 'capability' => Capabilities::STUDENT_PORTAL, 'label' => 'Subject Registration' ],
    ...
    public function register_rewrites(): void {
        ...
        add_rewrite_rule( '^verify-result/?$', 'index.php?' . self::QUERY_AREA . '=verify-result', 'top' );
    }
    ...
    public function dispatch(): void {
        ...
        // 0b — Result verification is a PUBLIC route. Anyone scanning the QR code
        //      on a report sheet should be able to verify it without logging in.
        if ( $area === 'verify-result' || ( $area === 'verify' ) ) {
    ...
    public static function navigation( string $area, string $current ): array {
        ...
        foreach ( self::sections()[ $area ] ?? [] as $slug => $section ) {
            ...
            // Exam officers should not see Remarks — that is a class-teacher task.
            if ( ! empty( $section['exclude_exam_officer'] ) && Gate::allows( Capabilities::MANAGE_PAPERS ) && ! Gate::allows( Capabilities::MANAGE_SCHOOL ) ) {
                continue;
            }
            ...
            // A teacher goes to this page to mark. A principal or exam officer goes
            // to it to see who still owes marking, so the same page needs a
            // different name depending on who is looking at it.
            if ( $slug === 'marking' && ( new Scope() )->is_school_wide() ) {
    ...
    /**
     * Public result verification page — reached by scanning the QR code on a
     * report sheet. Confirms the result exists and is published without
     * exposing the full report (which is behind the student/guardian login).
     */
    private function render_verification(): void {
        nocache_headers();

        $template = EDUCBT_PRO_PATH . 'templates/portal/account/verify.php';
    ...
    private function render( string $area, string $section ): void {
        // Every portal page is per-user: a dashboard, an inbox, a marking queue.
        // Nothing here may ever be cached. Without this, a host-level page cache
        // (LiteSpeed, Varnish, Cloudflare APO) serves one user's rendered page to
        // everyone and keeps serving it after the data changes — which looks
        // exactly like "my fix did not apply": the unread badge comes back, a new
        // class does not appear, an updated page still shows the old markup.
        nocache_headers();
        ...
        // LiteSpeed and WP Rocket listen for these rather than the constants.
        do_action( 'litespeed_control_set_nocache', 'EduCBT portal page' );
        ...
        foreach ( $candidates as $candidate ) {
            if ( file_exists( $candidate ) ) {
                // A stale copy of a template in the active theme silently wins over
                // the plugin's own, so plugin updates appear to do nothing. Say so
                // rather than leaving it to be guessed at.
                if ( strpos( $candidate, EDUCBT_PRO_PATH ) !== 0 && Gate::allows( Capabilities::MANAGE_SCHOOL ) ) {
                    echo '<div style="background:#fef3c7;border:1px solid #f59e0b;padding:10px;margin:10px;border-radius:8px;font:14px system-ui">'
                        . '<strong>Theme override in use.</strong> This page is being rendered from '
                        . esc_html( str_replace( ABSPATH, '', $candidate ) )
                        . ', not from the plugin. Plugin updates will not change this page until that file is removed.'
                        . '</div>';
                }
