...
class StudentRegistrationService {
    ...
    public function register( int $school_id, array $data ): array {
        ...
        // A school that already runs its own student IDs must be able to keep them.
        // Only generate when nothing was typed.
        $supplied = strtoupper( trim( sanitize_text_field( (string) ( $data['admission_number'] ?? '' ) ) ) );

        if ( $supplied !== '' ) {
            $clash = $wpdb->get_var(
                $wpdb->prepare(
                    'SELECT id FROM ' . $wpdb->prefix . 'educbt_students WHERE admission_number = %s LIMIT 1',
                    $supplied
                )
            );

            if ( $clash ) {
                return [ 'success' => false, 'errors' => [ 'admission_number' => 'admission_number_taken' ] ];
            }

            $admission_number = $supplied;
        } else {
