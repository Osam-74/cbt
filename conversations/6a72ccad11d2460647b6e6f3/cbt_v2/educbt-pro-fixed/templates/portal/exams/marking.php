    ...
    $staff_marking = (array) $wpdb->get_results(
        $wpdb->prepare(
            "SELECT st.id AS staff_id, st.wp_user_id,
             ...
             INNER JOIN {$attempts} at ON at.paper_id = p.id AND at.status = 'completed'
...
$educbt_body = static function () use ( $flash, $pending, $completed_exams, $queue, $progress, $paper_id, $is_wide, $staff_marking ): void {
